# Nimbusware — Nimbusware agent integration plan (local-first)

> **Nomenclature:** **Nimbusware** = this repository and product (API, Maker, Admin Console, config, IAM). **Nimbusware** = the online agentic system (adversarial pipeline, critics, gates, verifiers). See [ARCHITECTURE.md](ARCHITECTURE.md#nomenclature).

## Goal

Build **Nimbusware** as the local-first platform integrating the **Nimbusware** online agentic
...
hardware** through **many small, careful iterations** — not whole-repo rewrites. Nimbusware enforces
...
Nimbusware already consumes (persona shelves, role registry, workflow profiles, policy merge defaults,
...
**Nimbusware** is the operator product; **Nimbusware** remains the orchestration agent. **fo150–fo155**,
...
|------|-------------------|
| **fo150** | **Admin web operator chat** (Cursor-like) to configure runs and steer Nimbusware |
...
existing Nimbusware runs (micro-slice + gates), aimed at **business → software** on local hardware.
...
**Execution substrate (Lane M — ties to Nimbusware):**
...
- **Phase 4:** repo-scoped retrieval memory (`nimbusware_memory`), bundle usage memory, replay harness,
...
### 2.1 Control Plane (Nimbusware API + Nimbusware runtime)

Implemented as the Nimbusware FastAPI service (`nimbusware_api`) hosting Nimbusware orchestration.
...
connection managers). Nimbusware should prefer these reusable assets before full rewrites.
...
**Implementation:** validated in `nimbusware_extensions.personas`; promotion gaps in
...
| **pipeline** | ETL / batch jobs | Training/inference graph | CI stage graph / Nimbusware run stages |
...
**Distribution vs import package:** the Poetry project / wheel name may be `nimbusware` (or similar)
...
1. **Personas** ? `configs/personas/shelves.yaml` ? `nimbusware_config_personas` (or JSONB document per
   shelf); keep Pydantic validation in `nimbusware_extensions.personas`; API/Streamlit write Postgres;
   emit `persona.shelf.updated` (unchanged audit story).
2. **Role registry** ? extend today?s optional `nimbusware_roles_registry` to full registry metadata;
...
- Scraper response artifacts under `.cache/nimbusware_scraper` (?10).
...
**Goal:** full operator product (chat + custom agents) and Nimbusware runs that advance **one small
...
5. **fo154 — Context packets:** `SliceContextPacket` (capped paths, diff, test output, prior
   verdicts); LLM prompts assemble from packet when `NIMBUSWARE_USE_LLM=1`.
6. **fo155 — Diff-aware replan:** after implement, collect git/plan diff stats; on budget fail,
   ...
   env `NIMBUSWARE_SLICE_REPLAN_MAX`.
7. **Scoped slice.implement:** default `NIMBUSWARE_SLICE_IMPLEMENT=scoped` runs ruff format + `--fix` on
...
- Operator can chat to select/create an agent, start a `micro_slice` run, and see **≥2 slices**
  with per-slice gate outcomes in chat + timeline (`NIMBUSWARE_MICRO_SLICE_COUNT`, default `2`).
- `POST /v1/runs/{id}/lifecycle/verify` and `POST .../lifecycle/slice` run the automatic
  slice chain (`slice.plan` → `slice.gate`); LLM slice plans when `NIMBUSWARE_USE_LLM=1`.
...
|-------|----------------|
| **`packages/nimbusware_memory/`** | Nimbusware agent subsystem: chunking, embed, index build/search, sync protocol, retrieval API |
...
| **On-disk index (default)** | Under repo root, e.g. `configs/memory/index/` or `.nimbusware/memory/index/`: `faiss.index`, `memory_order.json`, `manifest.json` (mirrors bundle catalog layout) |
...
4. **Caps** — Reuse fo154 packet limits; add `memory_excerpt` with `NIMBUSWARE_MEMORY_RETRIEVAL_K` and
   `NIMBUSWARE_MEMORY_EXCERPT_MAX_CHARS` (names TBD in implementation).
5. **Nimbusware vs Nimbusware** — Logic in `nimbusware_memory` + `nimbusware_orchestrator`; operator toggles and
...
| **`ollama`** | When `NIMBUSWARE_USE_LLM=1` and workflow `memory.embedding_mode: ollama` with configured embedding model |
| **`sync`** | CLI (e.g. `nimbusware-memory-sync` or `nimbusware-config memory sync`) pulls/pushes **manifest + blobs** so a remote canonical store can hydrate **local FAISS** for full offline runs |
...
| **Orchestrator wiring** | `NIMBUSWARE_SLICE_IMPLEMENT=agent` path calling tool runtime; gates unchanged |
...
| **fo306** | **Agent tool runtime** — package + allowlist; wire `slice.implement` when `NIMBUSWARE_SLICE_IMPLEMENT=agent` |
...
- Ad-hoc ?processor language? for config?reuse Pydantic models from `nimbusware_extensions` / workflow parsers.
...
factory** (not a general chat workspace), with Nimbusware providing the adversarial agent runtime.
...
| Nimbusware wins when… | Nimbusware loses when… |
|-------------------|-------------------|
...
These extend capabilities Nimbusware already has; they are **higher priority** than feature parity
with chat UIs.
...
| **SWE-bench harness** | **fo452** | Optional benchmark job: Nimbusware `micro_slice` profile on public fixture repos (marketing + regression) |
...
| Gap vs peers | Learn from | Item | Epic | Normative outcome |
|--------------|------------|------|------|-------------------|
| No full sandbox | OpenHands, E2B | **Sandbox backend** | **fo480** | Optional per-run container/VM for `nimbusware_agent_tools` shell (policy-pluggable) |
...
**Implementation status (Jun 2026):** **Shipped (core ~92%)** in `packages/nimbusware_research/` — Maker Review + Admin run detail research/stitch panels; bundle catalog CRUD close-out in **A3**.

**Goal:** Before (and during) planning, Nimbusware **researches** comparable products and OSS
...
| **Persona shelves** | `nimbusware_extensions.personas` | Business-area researchers on **3B.1** shelf; Code Researcher on **3B.2**; Agent Evaluator may **spawn** domain shelf entries (?3B.4) |
...
| **Scraper stage** | `nimbusware_orchestrator` scraper mixin | Researchers request fetches via scraper; bodies land in `.cache/nimbusware_scraper/` or object store (fo204); **prompt_security** wrapper for untrusted HTML (? Odysseus-style policy in plan spirit) |
| **Executor** | `nimbusware_executor` | Role-gated egress only; researchers never bypass allowlist |
| **Event store** | `agent_core` + `nimbusware_store` | New append-only types (§20.7.5); extend PostgreSQL `CHECK` in same release |
| **Research pattern index** | `nimbusware_memory` (new namespace) | **Separate** from failure-memory FAISS: `configs/research/index/` or `nimbusware_research_chunk` table; embed `ResearchPattern` excerpts for Planner retrieval |
| **Failure memory** | Phase 4 `nimbusware_memory` | Unchanged; stitch failures also emit `finding.created` → future failure memory |
| **Bundle catalog** | `nimbusware_extensions.catalog` | Code Researcher may **promote** external repo into catalog candidate; Stitcher may consume **either** external manifest or `bundle_id` |
| **Bundle memory** | `nimbusware_bundle_outcome` | Stitcher transplants from bundles record outcomes same as integrator gate |
...
| **Agent tools** | `nimbusware_agent_tools` | Stitcher uses allowlisted read/write/copy; not unrestricted shell |
...
| **fo514** | Research pattern index (`nimbusware_memory` namespace or `packages/nimbusware_research/`) + `research.pattern.indexed` |
...
| `nimbusware_orchestrator.preflight` | Model latency p95, context length, JSON probe (?4.4) |
...
- macOS: Cookbook serve via vLLM limited; **Ollama + Metal** is the supported local path (Nimbusware + Nimbusware already align here).
...
- Postgres table `nimbusware_hardware_profile` **or** repo-local `.nimbusware/hardware/profile.json` (Individual default).
...
| Postgres `nimbusware_config_document` namespace `resource_governor` | Operator defaults (T2, ?19.5) |
...
POST /v1/platform/models/apply-preset:
  ...
  # writes nimbusware_config_document; returns materialize hint
```
...
operator chat**, which is user → Nimbusware steering).
