...
| `tests/e2e/web/` | Playwright smoke, apply-slice, session hub, launch scorecard (dimension rows), Settings launch check, Admin launch scorecard + factory evidence, scoped compaction toolbar, campaign progress + full replay, mobile theater parity (**24** tests / **18** spec files; sets `NIMBUSWARE_API_BASE` to test server port) |
