...
Package map: [packages/README.md](../packages/README.md). Import boundaries: [ARCHITECTURE.md](../ARCHITECTURE.md#import-rules-enforced).
...
| `tests/e2e/web/` | Playwright smoke, Chat patch/override/branch/escalation/theater, Admin operator chat classifier cards, Admin hardware catalog strip, Admin fleet hardware table + rescan, Admin run-detail panels + audit export, Admin config/fleet/nav smoke, Admin critic packs config tab, home intents + factory hero demos, Progress gate-fail action cards, Chat autopilot ladder hint, **solo hat coach hint** (`maker_solo_hat_coach.spec.ts`), Settings hybrid routing presets, apply-slice, session hub, launch scorecard (dimension rows + dev-env merge), Settings launch check + chat resume, scoped compaction toolbar, operator ribbons (incl. variant arena fitness + crossover merged), memory influence table, council ribbon gaps, variant fitness, campaign progress + full replay, mobile theater parity, **per-archetype product journey** (`maker_archetype_product_journey.spec.ts`), **button click expansion** (`maker_button_clicks.spec.ts`, `maker_playwright_extended.spec.ts`, `admin_button_clicks.spec.ts`; shared seed: `playwright_seed.ts`) (**106** tests / **60** spec files; **4** workers, **1** retry, **120s** timeout; sets `NIMBUSWARE_API_BASE` to test server port). Button inventory: [`tests/web/playwright_button_inventory.yaml`](web/playwright_button_inventory.yaml), gate: [`docs/testing/playwright-button-coverage.md`](../docs/testing/playwright-button-coverage.md). Parity matrix rows: readiness CTAs, Review open-PR CTA, Progress gate_summary, memory influence + council + variant (`parity_progress_wiring.yaml`), IDE MCP classify/patch/interject/fork, hybrid routing API (`test_routing_presets_api.py`) |
...
- **Playwright button gate:** `scripts/ci/run_playwright_button_ci_gate.py` (inventory freshness + click wiring contract; see [`docs/testing/playwright-button-coverage.md`](../docs/testing/playwright-button-coverage.md)).
