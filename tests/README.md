...
- **Local / PR parity:** `scripts/ci_check.ps1` or `ci_check.sh` — `ruff check`, advisory `ruff format --check` (non-blocking until backlog is reformatted), mypy (`services/` + tranche B), bandit, `pip-audit`, package coverage floors (`scripts/coverage_package_floors.py`), then pytest.
