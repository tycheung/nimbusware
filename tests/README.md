...
Pytest discovers **3,663** items under `tests/` with `pythonpath = ["packages", "tests"]` (see root `pyproject.toml`). The PR **unit** CI subset runs **~2,803** tests at **≥75%** coverage (82% total line coverage as of Jun 2026). Fixture repos under `tests/fixtures/repos/` are excluded from collection (`norecursedirs`). Playwright specs use a **120s** timeout (`tests/e2e/web/playwright.config.ts`) so campaign/API flows stay stable under CI parallel workers.
...
- **Framework pack PR gate:** `scripts/run_framework_pack_ci_gate.py` in default PR **unit** job (unit smoke + detect + PUT-preview for seven SPA packs + static HTML + write-replan + unknown-SPA journeys with keyboard/mouse fidelity; installs Chromium via `playwright` dev dependency).
