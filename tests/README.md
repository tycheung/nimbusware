...
| `tests/e2e/web/` | Playwright smoke, Chat patch/override/branch/escalation/theater, apply-slice, session hub, launch scorecard (dimension rows + dev-env merge), Settings launch check + chat resume, Admin launch scorecard + factory evidence, scoped compaction toolbar, operator ribbons (incl. variant arena), campaign progress + full replay, mobile theater parity (**39** tests / **27** spec files; sets `NIMBUSWARE_API_BASE` to test server port). Parity matrix rows: Home intent + factory hero demos, readiness CTAs, Review open-PR CTA, Progress gate-fail actions + gate_summary, IDE MCP classify/patch/interject/fork, hybrid routing API (`test_routing_presets_api.py`) |
