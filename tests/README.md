...
| `tests/fixtures/factory/` | Factory golden replay manifest (`golden_factory_replay_manifest.json` — crm, contacts, todo, static_site) + legacy `golden_factory_replay.json` |
| `tests/web/` | Web UI parity matrix (`@pytest.mark.web`) + launch wiring manifest (`parity_launch_wiring.yaml`) |
| `tests/e2e/web/` | Playwright smoke, apply-slice, session hub, launch scorecard (dimension rows), Settings launch check, Admin launch scorecard, campaign progress + full replay, mobile theater parity (22 specs; sets `NIMBUSWARE_API_BASE` to test server port) |
