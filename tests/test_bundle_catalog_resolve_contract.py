...
[`assert_bundle_catalog_maps_resolve`](d:\\Hermes\\packages\\hermes_orchestrator\\ingress.py)
wrapper-level coverage to keep that slice at exactly 3 parts. The
deferred surface item was item (2) of the fo81 Next slice block:

> wrapper-level negative coverage -- the inner helper
> ``assert_workflow_bundle_map_ids_resolve`` is well-tested but the
> wrapper-level ``FileNotFoundError`` (missing
> ``configs/bundles/catalog.yaml``) and ``ValueError`` (helper
> propagation) axes through ``ingress.assert_bundle_catalog_maps_resolve``
> are deferred from fo81 to keep it at 3 parts; a tight 2-axis
> test-only slice closes it.

fo82 closes both that wrapper-level gap AND extends the inner
helper's reject + early-return matrix.

Current coverage map at
...
only the "unknown bundle id" axis is tested.
