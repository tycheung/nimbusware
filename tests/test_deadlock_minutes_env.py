"""``HERMES_DEADLOCK_ESCALATION_MINUTES`` env-layer Pattern B int + fail-RAISE (fo72, §14 #19).
...
def test_deadlock_minutes_env_accept_arm_int_contract(
    ...
    2. **Zero boundary** -- ``"0"`` returns ``0`` literally; downstream
       ...
       [anti_deadlock.py](d:\\Hermes\\packages\\hermes_orchestrator\\anti_deadlock.py)
