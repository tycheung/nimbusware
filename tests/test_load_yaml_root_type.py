...
the **shared YAML loader** under every workflow / config parser in the
orchestrator:
...
via a ``try/except (OSError, ValueError, UnicodeDecodeError)`` pattern
that swallows ``load_yaml``'s ``ValueError`` and returns the parser's
default. The outer ``if isinstance(raw, dict)`` guards in both parsers
are therefore **dead code** under the current ``load_yaml`` contract:
``raw`` is guaranteed to be ``dict`` by the time the guard executes.

This is the **precursor to the fo76 zero-behavior cleanup** that
removes those dead guards. The Part C cascade tests below are the
regression suite that proves behavior is preserved across the cleanup.
...
``<block> parser=<name>: <component>`` identify the offending YAML
body and which contract axis broke.
"""
