...
with an early-fail-order assertion (``assert_known_workflow`` raises
-> no events appended to ``InMemoryEventStore``). fo81 and fo82
pinned wrapper-level / inner-helper contracts for gates 2, 3, and 5
but explicitly deferred the **cross-gate ordering meta-contract**:
nothing pins that gate N fires BEFORE gate N+1, and nothing extends
the early-fail-order guarantee to gates 2-5.

A future refactor could silently re-order gates, accidentally call
gate N+1 before gate N, or move a gate AFTER ``self._store.append``
-- breaking the architectural guarantee that failed ``create_run``
calls leave no partial state -- and none of the existing tests
would catch it.

fo83 closes that gap via three properties, all pinned by
monkeypatching the 5 gate references in the
``hermes_orchestrator.pipeline`` namespace (where ``create_run``
