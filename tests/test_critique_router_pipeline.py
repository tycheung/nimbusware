...
def test_implementation_critique_stub_off_by_default(
    ...
    assert not any(
        (r.get("payload") or {}).get("evidence_refs") == ["stub://implementation"]
        for r in rows
...
def test_test_writer_stub_emits_panel_after_verify(
    ...
    tw_critics = [
        ...
        for r in rows
        if r.get("event_type") == "critic.verdict.emitted"
        and (r.get("payload") or {}).get("evidence_refs") == ["stub://test_writer"]
    ]
    assert len(tw_critics) == 2
    gates = [
        r
        for r in rows
        if r.get("event_type") == "gate.decision.emitted"
...
def test_test_writer_skips_stub_when_llm_path_succeeds(
    ...
    rows = mem.list_run_events(str(rid))
    assert not any(
        (r.get("payload") or {}).get("evidence_refs") == ["stub://test_writer"]
        for r in rows
        if r.get("event_type") == "critic.verdict.emitted"
    )
...
def test_profile_stub_critique_yaml_without_env_emits_five_panels(
    ...
    orch, mem = make_dev_orchestrator()
    rid = orch.create_run("universal_critique_stub_on")
    orch.execute_writer_verifier_pass(rid)
    rows = mem.list_run_events(str(rid))

    def _stub_count(uri: str) -> int:
        return sum(
            1
            for r in rows
            if r.get("event_type") == "critic.verdict.emitted"
...
def test_stub_impl_env_false_overrides_yaml_stub_true(
    ...
    assert len(impl_stubs) == 0
    pll = [
        r
        for r in rows
        if r.get("event_type") == "critic.verdict.emitted"
        and (r.get("payload") or {}).get("evidence_refs") == ["stub://planner"]
    ]
    assert len(pll) == 2
...
def test_planner_skips_stub_when_llm_path_succeeds(
    ...
    rows = mem.list_run_events(str(rid))
    assert not any(
        (r.get("payload") or {}).get("evidence_refs") == ["stub://planner"]
        for r in rows
        if r.get("event_type") == "critic.verdict.emitted"
    )
...
def test_hard_block_off_runs_integrator_and_self_refinement(
    ...
    orch, mem = make_dev_orchestrator()
    rid = orch.create_run("universal_critique_hard_block_on")
    orch.execute_writer_verifier_pass(rid)
    rows = mem.list_run_events(str(rid))
    integrator_gates = [
        r
        ...
        and (r.get("metadata") or {}).get("integrator_gate") is True
    ]
    assert len(integrator_gates) == 1
    assert any(
...
def test_hard_block_on_skips_integrator_and_self_refinement(
    ...
    mock_emit_stub.side_effect = emit_stub_implementation_critique_panel_fail
    orch, mem = make_dev_orchestrator()
    rid = orch.create_run("universal_critique_hard_block_on")
    orch.execute_writer_verifier_pass(rid)
    rows = mem.list_run_events(str(rid))
    ...
    assert any(
        r.get("event_type") == EventType.GATE_DECISION_EMITTED.value
        and (r.get("payload") or {}).get("stage_name") == IMPLEMENTATION_CRITIQUE_STAGE
        and (r.get("payload") or {}).get("verdict") in (Verdict.FAIL, "FAIL")
        for r in rows
    )
...
def test_short_circuit_skips_test_writer_and_planner_after_impl_hard_block_fail(
    ...
    mock_emit_impl.side_effect = emit_stub_implementation_critique_panel_fail
    orch, mem = make_dev_orchestrator()
    rid = orch.create_run("universal_critique_hard_block_chain_on")
    orch.execute_writer_verifier_pass(rid)
    rows = mem.list_run_events(str(rid))
    critique_stages = [
    ...
    assert IMPLEMENTATION_CRITIQUE_STAGE in critique_stages
    assert TEST_WRITER_CRITIQUE_STAGE not in critique_stages
    assert PLANNER_CRITIQUE_STAGE not in critique_stages
...
def test_short_circuit_skips_planner_after_test_writer_hard_block_fail(
    ...
    mock_emit_tw.side_effect = emit_stub_test_writer_critique_panel_fail
    orch, mem = make_dev_orchestrator()
    rid = orch.create_run("universal_critique_tw_hard_block_chain_on")
    orch.execute_writer_verifier_pass(rid)
    rows = mem.list_run_events(str(rid))
    critique_stages = [
        (r.get("payload") or {}).get("stage_name")
    ...
    assert IMPLEMENTATION_CRITIQUE_STAGE in critique_stages
    assert TEST_WRITER_CRITIQUE_STAGE in critique_stages
    assert PLANNER_CRITIQUE_STAGE not in critique_stages
...
def test_hard_block_chain_profile_runs_all_critique_panels_when_impl_passes(
    ...
    assert IMPLEMENTATION_CRITIQUE_STAGE in critique_stages
    assert TEST_WRITER_CRITIQUE_STAGE in critique_stages
    assert PLANNER_CRITIQUE_STAGE in critique_stages
    assert FRONTEND_WRITER_CRITIQUE_STAGE in critique_stages
    assert MODULE_INTEGRATOR_CRITIQUE_STAGE in critique_stages
