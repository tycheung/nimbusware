...
def test_default_paths_shape_and_suffix_wire_format_5_axis(tmp_path: Path) -> None:
    ...
    assert isinstance(paths, tuple), (
        ...
        f"(packages/orchestrator/pipeline.py) "
    ...
    for idx, p in enumerate(paths):
        assert isinstance(p, Path), (
            ...
            f"[pipeline.py:143-144](packages/orchestrator/pipeline.py). "
...
def test_default_paths_root_resolution_5_axis(tmp_path: Path) -> None:
    ...
    assert (resolved_root / "configs").is_dir(), (
        ...
        f"located at ``packages/orchestrator/pipeline.py``. "
    ...
    assert (resolved_root / "packages" / "orchestrator" / "pipeline.py").is_file(), (
        f"B3: as a paired sanity check, the resolved root must contain "
        f"``packages/orchestrator/pipeline.py`` (the source of the "
...
def test_factory_cross_helper_key_divergences_5_axis(tmp_path: Path) -> None:
    ...
    assert orch._base_path == paths[0], (
        ...
        f"(packages/orchestrator/pipeline.py) -- the FIRST path "
        ...
        f"(packages/orchestrator/pipeline.py) would load workflow "
    ...
    assert orch.repo_root == via_first, (
        ...
        f"[pipeline.py:1567](packages/orchestrator/pipeline.py) "
    ...
    assert all("roles.yaml" not in s for s in paths_str), (
        ...
        f"[pipeline.py:1569](packages/orchestrator/pipeline.py) "
    ...
    assert "roles.yaml" in str(exc_d5.value), (
        ...
        f"at [pipeline.py:1569](packages/orchestrator/pipeline.py) / "
        f"[registry.py:26](packages/orchestrator/registry.py). "
