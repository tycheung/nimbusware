"""``RunOrchestrator.create_run`` construction-segment contract (fo86).

fo80 / fo83 / fo85 closed the three preceding segments of
``create_run``. The remaining unpinned block at
[pipeline.py:168-198](d:\\Hermes\\packages\\hermes_orchestrator\\pipeline.py)
constructs and appends the ``RunCreatedEvent``:

.. code-block:: python

    run_id = uuid4()                                          # line 168
    wf_path = workflow_profile_path(self._repo_root, workflow_profile)
    snapshot = policy_snapshot_from_files(                    # line 170
        self._base_path, wf_path, run_policy_overrides,
    )
    ev = RunCreatedEvent(
        ...
        correlation_id=corr,
        metadata={                                            # lines 181-190
            "roles_registry": {
                "yaml_version": self._registry.yaml_version,
                "content_digest_sha256_16": self._registry.content_digest_sha256_16,
            },
            "policy_snapshot": {
                "domain_allowlist_normalized": True,           # literal constant
                "network_egress_domain_count": len(
                    snapshot.network_egress.domain_allowlist,
                ),
            },
        },
        payload=RunCreatedPayload(                            # lines 191-196
            workflow_profile=workflow_profile,
            policy_version="1",                                # literal constant
...
The fo85 deferral note explicitly surfaced the
``run_policy_overrides`` <-> idempotency interaction as
out-of-scope; fo86 closes it together with the metadata/payload
shape contract.

fo86 closes the gap via 3 parts spanning 11 contract axes:

* **Part A** locks the 4 metadata axes at lines 181-190:

  - ``roles_registry.yaml_version`` equals ``orch._registry.yaml_version`` (int).
  - ``roles_registry.content_digest_sha256_16`` is a 16-hex-char string
    matching ``orch._registry.content_digest_sha256_16`` and is
    identical across two orchestrators built on the same repo
    (deterministic SHA-256-truncated digest).
  - ``policy_snapshot.domain_allowlist_normalized`` is the literal
    ``True`` regardless of workflow / overrides (pins line 187).
  - ``policy_snapshot.network_egress_domain_count`` equals
    ``len(snapshot.network_egress.domain_allowlist)`` (0 for the
    bare ``default`` workflow; 2 with non-empty override).

* **Part B** locks the 4 payload axes at lines 191-196:

  - ``payload.workflow_profile`` echoes the input arg for both
    ``default`` and ``agent_evaluator_on``.
  - ``payload.policy_version`` is the literal ``"1"``.
  - ``payload.config_snapshot_id`` is a parseable UUID string and
    two consecutive calls on the same orchestrator produce
    **different** values (proves ``str(uuid4())`` runs per-call).
  - ``payload.policy_snapshot.network_egress.domain_allowlist``
    reflects the merged + normalized snapshot when overrides
    supply domains (pins line 170 -> line 195 propagation).

* **Part C** locks the 3 ``run_policy_overrides`` axes:

  - ``None`` vs ``{}`` are equivalent no-ops for the merge.
  - Non-empty override propagates into both the stored
    snapshot AND the metadata count.
  - **STRONG cross-fo85 pin**: when call 2 short-circuits via
    idempotency (same ``correlation_id`` as call 1), the call-2
    overrides are silently dropped. The stored single row's
    ``domain_allowlist`` matches call-1 overrides only -- call-2
    beta overrides never reach ``policy_snapshot_from_files``
    because line 162-166 short-circuits before line 170.

Cross-slice symmetry (the complete ``create_run`` matrix):

| Slice    | Segment                                | Coverage                                  |
|----------|----------------------------------------|-------------------------------------------|
| fo80 PtB | pipeline.py:154 (gate 1)               | propagation                               |
| fo83     | pipeline.py:154-158 (5 gates)          | forward order + short-circuit             |
| fo85     | pipeline.py:162-180 (idempotency)      | fresh + roundtrip + lockout               |
| **fo86** | **pipeline.py:168-198 (construction)** | **metadata + payload + overrides+lockout**|

fo80 + fo83 + fo85 + **fo86** together form the **complete**
``create_run`` contract matrix from entry to ``store.append``.
"""
