...
[merge.py:20](d:\\Hermes\\packages\\hermes_orchestrator\\merge.py).
When that call encounters **syntactically invalid YAML** (unterminated
flow mapping, tab indent, unclosed quoted scalar, undefined anchor,
etc.) PyYAML raises ``yaml.YAMLError`` or a subclass
(``scanner.ScannerError``, ``parser.ParserError``,
``composer.ComposerError``). The loader does NOT catch it -- the
exception propagates out unchanged.

Sibling parsers like ``parse_escalation_workflow_block`` in
[workflow_escalation.py](d:\\Hermes\\packages\\hermes_orchestrator\\workflow_escalation.py)
and ``parse_security_scan_metadata_on_verify_workflow`` in
[workflow_security_metadata.py](d:\\Hermes\\packages\\hermes_orchestrator\\workflow_security_metadata.py)
wrap their ``load_yaml`` call in
``try/except (OSError, ValueError, UnicodeDecodeError)``. The crucial
divergence: ``yaml.YAMLError`` is **NOT a subclass of ``ValueError``,
``OSError``, or ``UnicodeDecodeError``**. Direct probe:

```
YAMLError MRO: ['YAMLError', 'Exception', 'BaseException', 'object']
subclass of ValueError: False
subclass of OSError: False
subclass of UnicodeDecodeError: False
```

So malformed YAML in a workflow profile **propagates
``yaml.YAMLError`` out of the parser** rather than degrading gracefully
to the default. Operators see a crash with PyYAML's exception class
instead of the documented "missing/malformed profile -> default"
behavior.

This is the **structural inversion of fo75 Part C**:

| Slice | YAML root | Loader behavior | Sibling-parser behavior |
|-------|-----------|-----------------|-------------------------|
| fo75 C | Non-dict (list / scalar / null) | Raises ``ValueError`` | Cascades to default |
| fo76 C | Syntactically malformed | Raises ``yaml.YAMLError`` | Propagates to caller |
...
``DID NOT RAISE`` -- the refactor PR would update Part C to assert
cascade-to-default (mirroring fo75 Part C), making the test diff an
intentional, deliberate behavioral change record.
"""
