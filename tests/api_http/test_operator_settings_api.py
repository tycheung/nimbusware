...
def test_patch_user_settings(client: TestClient) -> None:
    r = client.patch(
        "/v1/settings/me",
        json={"values": {"NIMBUSWARE_SLICE_AUTO_ADVANCE": "0"}},
    )
    assert r.status_code == 200
    assert r.json()["values"]["NIMBUSWARE_SLICE_AUTO_ADVANCE"] == "0"
...
def test_patch_system_requires_admin(client: TestClient) -> None:
    ...
    r = client.patch(
        "/v1/settings/system",
        json={"values": {"NIMBUSWARE_RERESARCH_MISSING_CONTEXT": "1"}},
        headers={"X-Nimbusware-Admin-Token": token},
    )
    assert r.status_code == 200
    assert r.json()["values"]["NIMBUSWARE_RERESARCH_MISSING_CONTEXT"] == "1"
