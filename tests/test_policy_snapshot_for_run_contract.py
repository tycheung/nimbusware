"""``RunOrchestrator.policy_snapshot_for_run`` READ-side contract (fo87).

fo86 closed the **WRITE** side of the policy snapshot
(construction + store append at
[pipeline.py:168-198](d:\\Hermes\\packages\\hermes_orchestrator\\pipeline.py)).
The natural ladder is the **READ** side at
[pipeline.py:541-552](d:\\Hermes\\packages\\hermes_orchestrator\\pipeline.py):
    ...
    def policy_snapshot_for_run(self, run_id: UUID) -> dict[str, Any]:
        for row in self._store.list_run_events(str(run_id)):
            if row["event_type"] == EventType.RUN_CREATED.value:
                ...
                if snap is None:                       # line 549-550
                    return {}
                return snap.model_dump(mode="json")    # line 551
        return {}                                       # line 552: no-match

This 11-line method is the inverse of fo86's write segment.
fo80 + fo83 + fo85 + fo86 pinned the construction pipeline
end-to-end; fo87 pins the read-back contract that downstream
code (``_strictness_context``, ``_effective_scraper_budget_bytes``)
depends on.

fo87 closes the gap via 3 parts spanning 11 contract axes:

* **Part A** locks the 3 empty / no-match axes (returns ``{}``):

  - Empty store with random run_id.
  - Different run_id when other runs exist (per-run lookup, not global).
  - Same run_id but only non-``RUN_CREATED`` rows (the event-type filter).

* **Part B** locks the 4 happy-path + fo86 round-trip axes:

  - Bare ``default`` workflow returns expected shape with nested keys.
  - Override propagates into returned dict's
    ``network_egress.domain_allowlist``.
  - **ROUND-TRIP EQUIVALENCE** (the fo86 cross-link): the stored
    payload's ``policy_snapshot`` dict equals the method's return.
    This is the central write=read pin.
  - Cross-workflow ``agent_evaluator_on`` returns expected shape.

* **Part C** locks the 4 degenerate-edge + downstream axes:

  - Hand-crafted ``RUN_CREATED`` with ``policy_snapshot=None``
    yields ``{}`` (pins line 549-550 None-branch).
  - Two ``RUN_CREATED`` rows for the same run_id -> the **first**
    in append order wins (pins iteration-order contract that
    downstream consumers depend on).
  - **Mutation safety**: mutating the returned dict does NOT
    affect future calls (pins ``model_dump(mode="json")`` fresh-
    dict contract combined with ``list_run_events`` ``deepcopy``).
  - ``_strictness_context`` downstream integration (proves the
    consumer that backs ``_emit_critique_gate_fail_findings``
    reads through this method).

Cross-slice symmetry (the complete write/read contract):

| Slice    | Segment                                | Coverage                                  |
|----------|----------------------------------------|-------------------------------------------|
| fo86     | pipeline.py:168-198 (construction)     | **WRITE**: metadata + payload + overrides |
| **fo87** | **pipeline.py:541-552 (read-back)**    | **READ**: empty + roundtrip + edges**    |

fo86 + **fo87** together form the **complete** write/read
contract for policy snapshots at the orchestrator boundary.
The B3 round-trip axis is the central pin proving the two
