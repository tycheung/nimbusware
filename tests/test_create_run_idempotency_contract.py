...
last per-gate wrapper coverage. The **post-gate idempotency block**
at
[pipeline.py:162-180](d:\\Hermes\\packages\\hermes_orchestrator\\pipeline.py)
remained unpinned:
...
passing ``correlation_id`` / ``idempotency_key`` kwargs. End-to-end
HTTP at [test_api.py:529-554](d:\\Hermes\\tests\\test_api.py)
covers only 2 axes (1 invalid-uuid 422 + 1 same-key roundtrip).
