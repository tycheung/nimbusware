...
from nimbusware_config.provider_connections import ProviderConnectionRow
...
def test_subscription_oauth_mock_flow_upserts_vault(
    ...
    monkeypatch.setattr(
        "nimbusware_api.routes.provider_subscription_oauth._connection_store",
        lambda: mock_store,
    )
    start = client.get(
...
def test_subscription_honor_system_link_works_when_oauth_configured(
    ...
    monkeypatch.setattr(
        "nimbusware_api.routes.provider_connections._store",
        lambda: mock_store,
    )
    res = client.post(
