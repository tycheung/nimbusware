...
* ``integrator_gate_timeline_summary`` (runs.py:48-74)
* ``agent_evaluator_timeline_summary`` (runs.py:80-101)
...
which exercise behavior through the FastAPI client + event store +
serialization. The defensive arms, identity-vs-truthy guards,
exact-vs-prefix stage matching, and cross-helper divergences are
unpinned at the unit level. OpenAPI ``RUN_TIMELINE_RESPONSE_200`` mirrors the same presence-gated policy (see ``openapi.py``). A repo-wide ``Grep`` for the six
function names returns only their definition site.

fo112 closes the gap with 4 parts spanning 20 axes (no source
changes):
