"""PZ-8: Phase 3 critic stages enabled on hermes_production."""
...
def test_hermes_production_yaml_enables_p3_blocks() -> None:
    root = Path(__file__).resolve().parents[1]
    sec = parse_security_critique_workflow_block(root, "hermes_production")
    perf = parse_performance_critique_workflow_block(root, "hermes_production")
    net = parse_network_resilience_critique_workflow_block(root, "hermes_production")
...
def test_hermes_production_verify_emits_p3_stages(monkeypatch: pytest.MonkeyPatch) -> None:
    root = Path(__file__).resolve().parents[1]
    orch, store = make_dev_orchestrator(root)
    run_id = orch.create_run("hermes_production")
