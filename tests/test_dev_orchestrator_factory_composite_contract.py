"""``default_paths`` + ``make_dev_orchestrator`` dev-factory composite (fo118).

Two sibling helpers at the end of
[packages/hermes_orchestrator/pipeline.py](packages/hermes_orchestrator/pipeline.py)
form the **dev-mode entry point** that anchors every direct-pipeline
test in the suite:
...
Existing coverage today is **invocation-only**: a repo-wide ``Grep``
finds ZERO references to ``default_paths`` in ``tests/``, and 300+
references to ``make_dev_orchestrator(`` -- each a factory call for
side effects (an orchestrator + a store), none a contract assertion
on the returned wiring, return shape, or path-resolution contract.

fo118 closes that gap with 4 parts spanning 20 axes (no source
changes):

* **Part A** -- ``default_paths`` shape + path-suffix wire-format
  (5 axes): 2-tuple arity, ``model-routing.yaml`` suffix,
  ``workflows/default.yaml`` suffix, ``Path``-not-``str`` type,
  override honored uniformly.
* **Part B** -- ``default_paths`` repo-root resolution (5 axes):
  ``None`` vs no-arg equivalence, absolute-path guarantee,
  ``configs/`` exists at ``parents[2]``, pure composition on a fresh
  ``tmp_path``, ``parent.parent`` invariant.
* **Part C** -- ``make_dev_orchestrator`` return shape + wiring
  (5 axes): 2-tuple arity, ``RunOrchestrator`` isinstance,
  ``InMemoryEventStore`` isinstance, ``orch.repo_root`` wiring,
  fresh-store distinctness across calls.
* **Part D** -- cross-helper KEY DIVERGENCES + invariants (5 axes):
  discarded-second-path wiring, repo-root re-derivation agrees,
  ``roles.yaml`` is the third hardcoded path NOT in ``default_paths``,
  ``default_paths`` purity on a nonexistent root,
  ``make_dev_orchestrator`` ``FileNotFoundError`` on a nonexistent
  root.

KEY DIVERGENCES pinned across the composite:

* **Second path discarded** -- ``make_dev_orchestrator`` unpacks
  ``base, _ = default_paths(root)`` (line 1568), throwing away the
  ``workflows/default.yaml`` path. A refactor swapping to
  ``_, base = default_paths(root)`` would silently wire
  ``RunOrchestrator(base_config_path=...)`` to the WRONG file. Part D
  D1 pins via ``orch._base_path == default_paths()[0]`` AND
  ``!= default_paths()[1]``.
* **Repo-root re-derivation, not delegation** --
  ``make_dev_orchestrator`` re-computes
  ``Path(__file__).resolve().parents[2]`` (line 1567) rather than
  delegating to ``default_paths(None)`` and walking back via
  ``.parent.parent``. The two sites must agree. Part D D2 pins their
  agreement via
  ``make_dev_orchestrator()[0].repo_root == default_paths()[0].parent.parent``
  AND ``== default_paths()[1].parent.parent.parent``. A refactor that
  drifted the ``parents[N]`` index in one of the two sites would
  break this.
* **Third hardcoded path** -- ``roles.yaml`` lives at
  ``root / "configs" / "roles.yaml"`` (line 1569) but is NOT in
  ``default_paths``'s 2-tuple. A refactor that broadened
  ``default_paths`` to a 3-tuple including ``roles.yaml`` would
  silently change the factory's expectations. Part D D3 pins that
  ``roles.yaml`` does NOT appear in any element of
  ``default_paths(...)``.
* **``default_paths`` is pure / ``make_dev_orchestrator`` is impure**
  -- ``default_paths`` just composes ``Path`` objects (no
  ``is_file()`` / ``read_bytes()`` calls), so it accepts any
  ``repo_root`` including nonexistent ones. ``make_dev_orchestrator``
  reads ``roles.yaml`` via
  ``RoleRegistry.from_yaml(...).read_bytes()``. Parts D4 / D5 pin
  this asymmetry side-by-side using the SAME nonexistent path: D4
  succeeds, D5 raises ``FileNotFoundError``.
"""
