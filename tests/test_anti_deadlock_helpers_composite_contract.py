...
tz-naive UTC normalization, unsorted ``store_seq``, ``int``-coerced
sort key on ``str`` ``store_seq``, ``count_progress_events`` boundary,
fail-open unknown event_type, ``_PROGRESS_IGNORE`` frozenset type
guarantee + exact 6-element membership, or cross-helper
``run.created`` dual-purpose interaction. fo116 closes these in 4
parts / 20 axes (source unchanged).
