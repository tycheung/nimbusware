...
3-arm loop at
[pipeline.py:901-958](d:\\Hermes\\packages\\hermes_orchestrator\\pipeline.py)
is only exercised one stage at a time in
[`tests/test_critique_router_pipeline.py`](d:\\Hermes\\tests\\test_critique_router_pipeline.py)
(impl-only / tw-only / planner-only). The 4 distinct ``continue``
arms inside the loop are mostly unpinned, the per-stage payload
shape has only spot-checks, and the 4 supporting pure helpers
have zero direct test coverage.

fo89 closes the gap via 4 parts spanning 16 contract axes:
