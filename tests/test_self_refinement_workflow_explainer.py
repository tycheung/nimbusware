...
def test_self_refinement_policy_yaml_file_bytes_caption(
    repo_sr_policy_and_workflow: Path,
) -> None:
...
def test_self_refinement_policy_yaml_disk_version_caption(
    repo_sr_policy_and_workflow: Path,
) -> None:
    out = self_refinement_workflow_explainer_payload(
        repo_sr_policy_and_workflow,
...
def test_self_refinement_merged_description_preview_caption() -> None:
    cap = self_refinement_merged_description_preview_caption(
        {
            "merged_description_preview": "override desc",
            "merged_description_len": 13,
        },
    )
    assert cap is not None
    ...
    long_text = "x" * 150
    cap_trunc = self_refinement_merged_description_preview_caption(
        {
            "merged_description_preview": long_text,
            "merged_description_len": 150,
        },
        max_chars=120,
    )
...
def test_self_refinement_would_emit_after_env_caption() -> None:
    cap_on = self_refinement_would_emit_after_env_caption(
        {"would_emit_marker_after_env": True},
    )
    assert cap_on is not None
    assert "would emit" in cap_on
    cap_off = self_refinement_would_emit_after_env_caption(
...
def test_self_refinement_would_emit_marker_caption() -> None:
    cap_on = self_refinement_would_emit_marker_caption(
        {
    ...
    assert cap_on is not None
    assert "would emit" in cap_on
    cap_env_off = self_refinement_would_emit_marker_caption(
        {
            "would_emit_self_refinement_marker": True,
            "would_emit_marker_after_env": False,
        },
    ...
    cap_off = self_refinement_would_emit_marker_caption(
        {
            "would_emit_self_refinement_marker": False,
            "would_emit_marker_after_env": False,
        },
    )
    assert cap_off is not None
...
def repo_sr_workflow_only(tmp_path: Path) -> Path:
    ...
    (tmp_path / "configs" / "workflows" / "wf.yaml").write_text(
        "version: 1\n"
        "self_refinement:\n"
        "  enabled: true\n"
        "  version: 2\n"
...
def test_would_emit_when_workflow_enables_even_if_policy_off(
    repo_sr_policy_and_workflow: Path,
) -> None:
...
def test_env_kill_switch_marks_effective_emit_false(
    ...
    (tmp_path / "configs" / "workflows").mkdir(parents=True)
    (tmp_path / "configs" / "workflows" / "w.yaml").write_text(
        "version: 1\nself_refinement:\n  enabled: false\n",
        encoding="utf-8",
    )
...
def test_marker_merge_vs_timeline_rows_no_timeline() -> None:
    rows = self_refinement_marker_merge_vs_timeline_rows(
        {"would_emit_self_refinement_marker": True, "would_emit_marker_after_env": True},
        None,
    )
...
def test_marker_merge_vs_timeline_version_mismatch() -> None:
    rows = self_refinement_marker_merge_vs_timeline_rows(
        {"merged_version": 1, "merged_description_len": 0},
        {"version": 2},
    )
...
def test_marker_merge_vs_timeline_from_explainer_payload(
    repo_sr_policy_and_workflow: Path,
) -> None:
    out = self_refinement_workflow_explainer_payload(
        repo_sr_policy_and_workflow,
...
def test_marker_merge_vs_timeline_with_full_timeline_paste_resolution() -> None:
    wall = {
        ...
        "events": [],
        "self_refinement": {"version": 5, "description": "hi!"},
    }
...
def test_self_refinement_explainer_table_rows_export_json_and_csv(
    repo_sr_policy_and_workflow: Path,
) -> None:
    ...
    rows = self_refinement_explainer_table_rows(payload)
    fields = {r["field"] for r in rows}
    assert "workflow_profile" in fields
    assert "self_refinement_yaml_present" in fields
    assert "marker_merge" in fields
    marker_row = next(r for r in rows if r["field"] == "marker_merge")
    assert "would_emit_self_refinement_marker" in marker_row["value"]
    assert len(rows) == len(payload)
    parsed = json.loads(self_refinement_explainer_export_json(payload))
    assert parsed["workflow_profile"] == "on"
    csv_text = self_refinement_explainer_table_rows_csv(rows)
    assert csv_text.splitlines()[0] == "field,value"
    assert self_refinement_explainer_table_rows({}) == []  # type: ignore[arg-type]
    assert self_refinement_explainer_table_rows_csv([]) == ""
    assert self_refinement_export_filename_slug() == "self_refinement"


def test_self_refinement_explainer_table_rows_minimal_fixture() -> None:
    payload = {
        "workflow_profile": "wf",
        "marker_merge": {"would_emit_self_refinement_marker": True},
    }
    rows = self_refinement_explainer_table_rows(payload)
    assert len(rows) == 2
    assert json.loads(self_refinement_explainer_export_json(payload)) == payload
...
def test_self_refinement_marker_merge_compare_export_json_and_csv() -> None:
    ...
    assert (
        self_refinement_marker_merge_compare_export_filename_slug()
        == "self_refinement_marker_compare"
    )
...
def test_self_refinement_workflow_explainer_payload_includes_ungated_env(
    repo_sr_policy_and_workflow: Path,
) -> None:
    out = self_refinement_workflow_explainer_payload(
        repo_sr_policy_and_workflow,
...
def test_self_refinement_workflow_explainer_operator_metrics() -> None:
    payload = {
        "self_refinement_yaml_present": True,
        "self_refinement_yaml_mapping_string_key_count": 3,
        "policy_yaml": {"enabled": True, "version": 2},
        "marker_merge": {
            "would_emit_self_refinement_marker": True,
            "would_emit_marker_after_env": False,
        },
...
def test_self_refinement_workflow_explainer_operator_metrics_export() -> None:
    m = self_refinement_workflow_explainer_operator_metrics(
        {"load_error": "missing"},
    )
    assert m["load_error_present"] is True
    rows = self_refinement_workflow_explainer_operator_metrics_table_rows(m)
