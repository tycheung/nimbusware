...
at [scraper_artifacts.py:10-15](d:\\Hermes\\packages\\hermes_orchestrator\\scraper_artifacts.py):
...
env-layer sweep started at fo62 (10 envs across two Patterns) and the
**first string-passthrough subtype**, closing the Pattern B subtype
quartet:
...
signals -- ``"true"`` -> ``Path("true").resolve()`` =
``<cwd>/true`` -- directly inverting the fo65-70 Pattern A binary
gates. A refactor that added a truthy-tuple short-circuit (e.g.
``if raw.lower() in ("1","true","yes"): return alt_default``) would
silently change operator-facing behavior for any operator with a
directory literally named ``true`` / ``yes`` / ``on``.
...
the offending env scalar and which axis broke.
"""
