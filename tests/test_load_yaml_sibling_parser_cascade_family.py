...
`parse_security_scan_metadata_on_verify_workflow`). fo76 Part C pinned
the structural inversion: `yaml.YAMLError` propagates rather than
cascading, again for the same 2 parsers. **4 more sibling parsers
share the identical `except (OSError, ValueError, UnicodeDecodeError)`
...
``self_refinement: []``, ``universal_critique: true``). **None** test
a non-dict **document** root or malformed YAML syntax.
...
| Slice | Parsers | Axis |
|-------|---------|------|
| fo75 Part C | 2 (esc, sec_meta) | Non-dict root -> cascade |
| fo76 Part C | 2 (esc, sec_meta) | Malformed -> ``yaml.YAMLError`` |
| **fo77 Part A** | **4 (ae, sr, uc, ig)** | **Non-dict root -> cascade** |
| **fo77 Part B** | **4 (same 4)** | **Malformed -> ``yaml.YAMLError``** |
| **fo77 Part C** | **All 6 (uniformity)** | **Both axes per parser** |

Together fo75 + fo76 + fo77 produce a complete 6 x 2 = 12-cell
cascade-family contract matrix. A future fo78 cascade-harden refactor
(adding ``yaml.YAMLError`` to all 6 parsers' except tuples in lockstep)
would flip Part B's ``pytest.raises`` to cascade-to-default and Part C
would diff in lockstep, making the test diff the intentional behavioral
change record.
"""
