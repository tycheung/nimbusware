...
def test_sak520_b_openapi_bootstrap_overlap_503() -> None:
    ...
    assert "sak520-b" in (_ROOT / "packages" / "api" / "routes" / "web_bootstrap.py").read_text(
        encoding="utf-8"
    )
    assert "sak520-b" in (_ROOT / "packages" / "api" / "routes" / "admin_ui_bff.py").read_text(
        encoding="utf-8"
    )
...
def test_sak520_c_openapi_fleet_autopilot_admin_ui_503() -> None:
    ...
    assert "sak520-c" in (_ROOT / "packages" / "api" / "routes" / "admin_ui_bff.py").read_text(
        encoding="utf-8"
    )
...
def test_sak520_g_openapi_fleet_enforcement_get_503() -> None:
    ...
    assert "sak520-g" in (_ROOT / "packages" / "api" / "routes" / "admin_ui_bff.py").read_text(
        encoding="utf-8"
    )
...
def test_sak520_h_openapi_fleet_enforcement_put_503() -> None:
    ...
    assert "sak520-h" in (_ROOT / "packages" / "api" / "routes" / "admin_ui_bff.py").read_text(
        encoding="utf-8"
    )
...
def test_sak520_i_openapi_push_subscriptions_mutate_503() -> None:
    ...
    assert "sak520-i" in (_ROOT / "packages" / "api" / "routes" / "maker_push.py").read_text(
        encoding="utf-8"
    )
