...
def test_agent_evaluator_env_gate_caption() -> None:
    ...
    cap_on = agent_evaluator_env_gate_caption({'NIMBUSWARE_AGENT_EVALUATOR': {'forces_on': True, 'raw': '1'}})
    assert cap_on is not None
    assert 'force-on' in cap_on
    cap_unset = agent_evaluator_env_gate_caption({'NIMBUSWARE_AGENT_EVALUATOR': {'unset': True}})
...
def test_agent_evaluator_auto_promote_env_gate_caption() -> None:
    cap_off = agent_evaluator_auto_promote_env_gate_caption({'NIMBUSWARE_AGENT_EVALUATOR_AUTO_PROMOTE': {'disables_auto_promote': True, 'raw': '0'}})
    assert cap_off is not None
    assert 'kill-switch' in cap_off
    cap_unset = agent_evaluator_auto_promote_env_gate_caption({'NIMBUSWARE_AGENT_EVALUATOR_AUTO_PROMOTE': {'unset': True}})
    assert cap_unset is not None
    assert 'unset' in cap_unset
    cap_bad = agent_evaluator_auto_promote_env_gate_caption({'NIMBUSWARE_AGENT_EVALUATOR_AUTO_PROMOTE': {'unrecognised_value': True, 'raw': 'maybe'}})
...
def test_agent_evaluator_auto_create_env_gate_caption() -> None:
    cap_off = agent_evaluator_auto_create_env_gate_caption({'NIMBUSWARE_AGENT_EVALUATOR_AUTO_CREATE': {'disables_auto_create': True, 'raw': '0'}})
    assert cap_off is not None
    assert 'kill-switch' in cap_off
    cap_unset = agent_evaluator_auto_create_env_gate_caption({'NIMBUSWARE_AGENT_EVALUATOR_AUTO_CREATE': {'unset': True}})
    assert cap_unset is not None
    assert 'unset' in cap_unset
...
def test_agent_evaluator_yaml_key_present_caption() -> None:
    cap_absent = agent_evaluator_yaml_key_present_caption({'agent_evaluator_yaml_key_present': False})
    assert cap_absent is not None
    assert 'absent' in cap_absent
    cap_on = agent_evaluator_yaml_key_present_caption({'agent_evaluator_yaml_key_present': True, 'yaml_parsed_enabled': True})
    assert cap_on is not None
    assert 'present' in cap_on
    assert 'true' in cap_on
    cap_off = agent_evaluator_yaml_key_present_caption({'agent_evaluator_yaml_key_present': True, 'yaml_parsed_enabled': False})
    assert cap_off is not None
...
def test_agent_evaluator_persona_id_caption() -> None:
    cap = agent_evaluator_persona_id_caption({'yaml_parsed_persona_id': 'default'})
    assert cap is not None
    assert '`default`' in cap
    cap_custom = agent_evaluator_persona_id_caption({'yaml_parsed_persona_id': '  custom-p  '})
    ...
    assert agent_evaluator_persona_id_caption({'yaml_parsed_persona_id': ''}) is None
    assert agent_evaluator_persona_id_caption({'yaml_parsed_persona_id': '   '}) is None

def test_agent_evaluator_yaml_parsed_enabled_caption() -> None:
    cap_on = agent_evaluator_yaml_parsed_enabled_caption({'agent_evaluator_yaml_key_present': True, 'yaml_parsed_enabled': True})
    assert cap_on is not None
    assert '**true**' in cap_on
    cap_off = agent_evaluator_yaml_parsed_enabled_caption({'agent_evaluator_yaml_key_present': True, 'yaml_parsed_enabled': False})
    assert cap_off is not None
    assert '**false**' in cap_off
    ...
    assert agent_evaluator_yaml_parsed_enabled_caption({'agent_evaluator_yaml_key_present': False, 'yaml_parsed_enabled': True}) is None
    assert agent_evaluator_yaml_parsed_enabled_caption({'agent_evaluator_yaml_key_present': True, 'yaml_parsed_enabled': 'yes'}) is None

def test_agent_evaluator_llm_evaluation_enabled_caption() -> None:
    cap_on = agent_evaluator_llm_evaluation_enabled_caption({'agent_evaluator_yaml_key_present': True, 'yaml_parsed_llm_evaluation_enabled': True})
    assert cap_on is not None
    assert 'llm_evaluation_enabled' in cap_on
    assert '**on**' in cap_on
    cap_off = agent_evaluator_llm_evaluation_enabled_caption({'agent_evaluator_yaml_key_present': True, 'yaml_parsed_llm_evaluation_enabled': False})
    assert cap_off is not None
    assert '**off**' in cap_off
    assert agent_evaluator_llm_evaluation_enabled_caption(None) is None

def test_agent_evaluator_workflow_explainer_payload_llm_evaluation_enabled(tmp_path: Path) -> None:
    write_workflow_profile(tmp_path, 'ae_llm', 'version: 1\nagent_evaluator:\n  enabled: true\n  llm_evaluation_enabled: true\n')
...
def test_agent_evaluator_yaml_raw_type_caption(tmp_path: Path) -> None:
    write_workflow_profile(tmp_path, 'ae', 'version: 1\nagent_evaluator:\n  enabled: true\n')
    pl = agent_evaluator_workflow_explainer_payload(tmp_path, workflow_profile='ae')
...
def test_agent_evaluator_workflow_yaml_version_caption() -> None:
    cap = agent_evaluator_workflow_yaml_version_caption({'workflow_yaml_top_level_version_int': 3})
    assert cap is not None
    assert '**3**' in cap
    assert agent_evaluator_workflow_yaml_version_caption(None) is None
    assert agent_evaluator_workflow_yaml_version_caption({'load_error': 'bad'}) is None
    assert agent_evaluator_workflow_yaml_version_caption({'workflow_yaml_top_level_version_int': 0}) is None
    assert agent_evaluator_workflow_yaml_version_caption({'workflow_yaml_top_level_version_int': True}) is None
    cap_one = agent_evaluator_workflow_yaml_version_caption({'workflow_yaml_top_level_version_int': 1})
...
def test_explainer_missing_agent_evaluator_key(tmp_path: Path) -> None:
    ...
    pl = agent_evaluator_workflow_explainer_payload(tmp_path, workflow_profile='bare')
    ...
    assert pl['yaml_parsed_auto_create_persona'] == {'enabled': False, 'shelf': '', 'display_name': ''}
...
def test_explainer_workflow_yaml_top_level_version_missing(tmp_path: Path) -> None:
    write_workflow_profile(tmp_path, 'no_ver', 'agent_evaluator:\n  enabled: false\n  persona_id: x\n')
    pl = agent_evaluator_workflow_explainer_payload(tmp_path, workflow_profile='no_ver')
    assert pl['workflow_yaml_top_level_version_int'] is None
    write_workflow_profile(tmp_path, 'ae_on', 'version: 1\nagent_evaluator:\n  enabled: true\n  persona_id: custom-p\n')
    pl = agent_evaluator_workflow_explainer_payload(tmp_path, workflow_profile='ae_on')
...
def test_explainer_workflow_disabled(tmp_path: Path) -> None:
    write_workflow_profile(tmp_path, 'ae_off', 'version: 1\nagent_evaluator:\n  enabled: false\n')
    pl = agent_evaluator_workflow_explainer_payload(tmp_path, workflow_profile='ae_off')
    assert pl['yaml_parsed_enabled'] is False
    assert pl['agent_evaluator_yaml_raw_type'] == 'dict'
...
def test_explainer_auto_promote_probation_yaml_true(tmp_path: Path) -> None:
    write_workflow_profile(tmp_path, 'ae_promo', 'version: 1\nagent_evaluator:\n  enabled: true\n  persona_id: x\n  auto_promote_probation: true\n')
    pl = agent_evaluator_workflow_explainer_payload(tmp_path, workflow_profile='ae_promo')
...
def test_explainer_auto_create_persona_yaml_block(tmp_path: Path) -> None:
    write_workflow_profile(tmp_path, 'ae_create', 'version: 1\nagent_evaluator:\n  enabled: true\n  persona_id: net_new_ae_pid\n  auto_create_persona:\n    enabled: true\n    shelf: business_area\n    display_name: Net New AE\n')
    pl = agent_evaluator_workflow_explainer_payload(tmp_path, workflow_profile='ae_create')
    ac = pl['yaml_parsed_auto_create_persona']
    assert ac == {'enabled': True, 'shelf': 'business_area', 'display_name': 'Net New AE'}
...
def test_explainer_env_kill_switch_overrides_workflow_on(monkeypatch: object, tmp_path: Path) -> None:
...
def test_agent_evaluator_workflow_explainer_operator_metrics_would_emit_llm() -> None:
    payload = {'would_emit_llm_evaluation': True, 'yaml_parsed_llm_evaluation_enabled': True, 'would_emit_stage_started': True}
...
def test_agent_evaluator_workflow_explainer_operator_metrics_llm_and_env_gates() -> None:
    payload = {'yaml_parsed_llm_evaluation_enabled': True, 'NIMBUSWARE_AGENT_EVALUATOR_AUTO_PROMOTE': {'disables_auto_promote': True}, 'NIMBUSWARE_AGENT_EVALUATOR_AUTO_CREATE': {'disables_auto_create': True}}
    m = agent_evaluator_workflow_explainer_operator_metrics(payload)
...
def test_agent_evaluator_workflow_explainer_operator_metrics_yaml_bool_counts() -> None:
    payload = {'agent_evaluator_yaml_true_bool_value_count': 2, 'agent_evaluator_yaml_false_bool_value_count': 1}
    m = agent_evaluator_workflow_explainer_operator_metrics(payload)
...
def test_agent_evaluator_workflow_explainer_operator_metrics_env_forces_on() -> None:
    payload = {'agent_evaluator_yaml_key_present': True, 'yaml_parsed_enabled': False, 'yaml_parsed_persona_id': 'p1', 'would_emit_stage_started': True, 'NIMBUSWARE_AGENT_EVALUATOR': {'forces_on': True, 'forces_off': False, 'unset': False}, 'workflow_yaml_top_level_version_int': 3}
    m = agent_evaluator_workflow_explainer_operator_metrics(payload)
