...
def test_explainer_escalation_policy_yaml_rejects_non_int_scalars(
    tmp_path: Path, policy_yaml: str, field: str, expected: object
) -> None:
...
def test_escalation_yaml_key_present_caption() -> None:
    ...
    cap_on = escalation_yaml_key_present_caption(
        {"escalation_yaml_key_present": True, "suppress_automatic_escalation_effective": True}
    )
    assert cap_on is not None
    assert "true" in cap_on
    cap_off = escalation_yaml_key_present_caption(
        {"escalation_yaml_key_present": True, "suppress_automatic_escalation_effective": False}
    )
...
def test_escalation_suppress_flag_caption_none_when_effective_non_bool() -> None:
    ...
    assert (
        escalation_suppress_flag_caption({**base, "suppress_automatic_escalation_effective": 1})
        is None
    )
    assert (
        escalation_suppress_flag_caption(
    ...
    assert (
        escalation_suppress_flag_caption({**base, "suppress_automatic_escalation_effective": None})
        is None
    )
...
def test_escalation_suppress_workflow_explainer_operator_metrics_policy_file_bytes() -> None:
    m = escalation_suppress_workflow_explainer_operator_metrics(
        {"escalation_policy_yaml_path_exists": True, "escalation_policy_yaml_file_bytes": 512}
    )
