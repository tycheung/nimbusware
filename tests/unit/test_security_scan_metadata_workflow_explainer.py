...
def test_security_scan_metadata_workflow_yaml_string_key_count_caption(tmp_path: Path) -> None:
    ...
    assert (
        security_scan_metadata_workflow_yaml_string_key_count_caption({"load_error": "bad"}) is None
    )
...
def test_security_scan_metadata_workflow_yaml_file_bytes_caption(tmp_path: Path) -> None:
    ...
    assert (
        security_scan_metadata_workflow_yaml_file_bytes_caption({"workflow_yaml_file_bytes": -1})
        is None
    )
    assert (
        security_scan_metadata_workflow_yaml_file_bytes_caption({"workflow_yaml_file_bytes": True})
        is None
    )
...
def test_explainer_dict_enabled_false(tmp_path: Path) -> None:
    ...
    pl = security_scan_metadata_workflow_explainer_payload(
        tmp_path, workflow_profile="sec_dict_off"
    )
...
def test_explainer_mapping_empty_counts_zero(tmp_path: Path) -> None:
    write_workflow_profile(
        tmp_path, "sec_empty_map", "version: 1\nsecurity_scan_metadata_on_verify: {}\n"
    )
    pl = security_scan_metadata_workflow_explainer_payload(
        tmp_path, workflow_profile="sec_empty_map"
    )
    assert pl["security_scan_metadata_on_verify_yaml_raw_type"] == "dict"
