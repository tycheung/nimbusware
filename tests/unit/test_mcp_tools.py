...
def test_hermes_run_status(mock_get: Any) -> None:
    mock_get.return_value = {"run_id": "abc", "status": "running"}
    out = call_tool("hermes_run_status", {"run_id": "abc"})
...
def test_hermes_approve_plan(mock_post: Any) -> None:
    mock_post.return_value = {"status": "approved"}
    out = call_tool("hermes_approve_plan", {"run_id": "abc"})
