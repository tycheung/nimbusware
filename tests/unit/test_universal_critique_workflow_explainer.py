...
def _clear_universal_critique_env(monkeypatch: pytest.MonkeyPatch) -> None:
    for key in ('NIMBUSWARE_ENABLE_TEST_WRITER_CRITIQUE', 'NIMBUSWARE_ENABLE_PLANNER_CRITIQUE', 'NIMBUSWARE_STUB_IMPLEMENTATION_CRITICS'):
        monkeypatch.delenv(key, raising=False)

@pytest.fixture()
def repo_uc_stub(tmp_path: Path) -> Path:
    (tmp_path / 'configs' / 'workflows').mkdir(parents=True)
    (tmp_path / 'configs' / 'workflows' / 'stub.yaml').write_text('version: 1\nuniversal_critique:\n  implementation:\n    llm: false\n    stub: true\n  test_writer:\n    enabled: true\n    llm: false\n    stub: true\n  planner:\n    enabled: false\n  frontend_writer:\n    enabled: true\n    llm: false\n    stub: true\n  module_integrator:\n    enabled: true\n    llm: false\n    stub: true\n', encoding='utf-8')
    return tmp_path

def test_explainer_yaml_and_effective_match_without_env(repo_uc_stub: Path) -> None:
    out = universal_critique_workflow_explainer_payload(repo_uc_stub, workflow_profile='stub')
    assert out['workflow_profile'] == 'stub'
    assert out['universal_critique_yaml_present'] is True
    assert out['universal_critique_yaml_top_level_keys'] == ['frontend_writer', 'implementation', 'module_integrator', 'planner', 'test_writer']
...
def test_explainer_top_level_scalar_leaf_counts_bool_and_string(tmp_path: Path) -> None:
    (tmp_path / 'configs' / 'workflows').mkdir(parents=True)
    (tmp_path / 'configs' / 'workflows' / 'scalars.yaml').write_text('version: 2\nuniversal_critique:\n  hard_block: true\n  note: "watch"\n  implementation:\n    enabled: false\n', encoding='utf-8')
    out = universal_critique_workflow_explainer_payload(tmp_path, workflow_profile='scalars')
...
def test_explainer_top_level_list_child_count(tmp_path: Path) -> None:
    (tmp_path / 'configs' / 'workflows').mkdir(parents=True)
    (tmp_path / 'configs' / 'workflows' / 'lists.yaml').write_text('version: 1\nuniversal_critique:\n  tags:\n    - alpha\n    - beta\n  implementation:\n    enabled: false\n', encoding='utf-8')
    out = universal_critique_workflow_explainer_payload(tmp_path, workflow_profile='lists')
...
def test_explainer_top_level_nonempty_count_skips_empty_planner(tmp_path: Path) -> None:
    (tmp_path / 'configs' / 'workflows').mkdir(parents=True)
    (tmp_path / 'configs' / 'workflows' / 'sparse.yaml').write_text('version: 1\nuniversal_critique:\n  implementation:\n    llm: false\n  planner: {}\n  test_writer:\n    enabled: true\n', encoding='utf-8')
    out = universal_critique_workflow_explainer_payload(tmp_path, workflow_profile='sparse')
    assert out['universal_critique_yaml_top_level_keys'] == ['implementation', 'planner', 'test_writer']
...
def test_universal_critique_workflow_yaml_relpath_caption(repo_uc_stub: Path) -> None:
    out = universal_critique_workflow_explainer_payload(repo_uc_stub, workflow_profile='stub')
    ...
    assert universal_critique_workflow_yaml_relpath_caption({'load_error': 'bad'}) is None
    assert universal_critique_workflow_yaml_relpath_caption({}) is None

def test_universal_critique_yaml_top_level_nonempty_count_caption(repo_uc_stub: Path) -> None:
    out = universal_critique_workflow_explainer_payload(repo_uc_stub, workflow_profile='stub')
    ...
    assert universal_critique_yaml_top_level_nonempty_count_caption({'load_error': 'bad'}) is None
    assert universal_critique_yaml_top_level_nonempty_count_caption({'universal_critique_yaml_present': False}) is None

def test_universal_critique_yaml_top_level_enabled_true_count_caption(repo_uc_stub: Path) -> None:
    out = universal_critique_workflow_explainer_payload(repo_uc_stub, workflow_profile='stub')
...
def test_universal_critique_yaml_top_level_enabled_false_count_caption(repo_uc_stub: Path) -> None:
    out = universal_critique_workflow_explainer_payload(repo_uc_stub, workflow_profile='stub')
    ...
    assert universal_critique_yaml_top_level_enabled_false_count_caption({'universal_critique_yaml_present': False}) is None

def test_universal_critique_yaml_top_level_mapping_child_count_caption(repo_uc_stub: Path) -> None:
    out = universal_critique_workflow_explainer_payload(repo_uc_stub, workflow_profile='stub')
    ...
    assert universal_critique_yaml_top_level_mapping_child_count_caption({'load_error': 'bad'}) is None

def test_universal_critique_yaml_top_level_list_child_count_caption(repo_uc_stub: Path) -> None:
    out = universal_critique_workflow_explainer_payload(repo_uc_stub, workflow_profile='stub')
...
def test_universal_critique_workflow_yaml_bytes_caption(repo_uc_stub: Path) -> None:
    out = universal_critique_workflow_explainer_payload(repo_uc_stub, workflow_profile='stub')
    ...
    assert universal_critique_workflow_yaml_bytes_caption({'load_error': 'bad'}) is None
    assert universal_critique_workflow_yaml_bytes_caption({}) is None
    assert universal_critique_workflow_yaml_bytes_caption({'universal_critique_workflow_yaml_bytes': -1}) is None
    assert universal_critique_workflow_yaml_bytes_caption({'universal_critique_workflow_yaml_bytes': True}) is None

def test_universal_critique_yaml_present_caption() -> None:
    cap_absent = universal_critique_yaml_present_caption({'universal_critique_yaml_present': False})
    assert cap_absent is not None
    assert 'absent' in cap_absent
    cap_present = universal_critique_yaml_present_caption({'universal_critique_yaml_present': True, 'universal_critique_yaml_top_level_keys': ['a', 'b']})
    ...
    assert universal_critique_yaml_present_caption({'load_error': 'bad'}) is None

def test_universal_critique_default_enabled_caption() -> None:
    root = find_repo_root(start=Path(__file__).resolve().parents[1])
    payload = universal_critique_workflow_explainer_payload(root, workflow_profile='universal_critique_on')
    cap = universal_critique_default_enabled_caption(payload)
    assert cap is not None
    assert 'default_enabled' in cap
    assert '**on**' in cap
    off_payload = universal_critique_workflow_explainer_payload(root, workflow_profile='default')
...
def test_universal_critique_yaml_enabled_bucket_caption() -> None:
    cap = universal_critique_yaml_enabled_bucket_caption({'universal_critique_yaml_present': True, 'universal_critique_yaml_top_level_enabled_true_count': 2, 'universal_critique_yaml_top_level_enabled_false_count': 1, 'universal_critique_yaml_top_level_enabled_unset_mapping_count': 0})
    assert cap is not None
    assert '**2** true' in cap
    assert '**1** false' in cap
    assert universal_critique_yaml_enabled_bucket_caption(None) is None
    assert universal_critique_yaml_enabled_bucket_caption({'universal_critique_yaml_present': False}) is None
    assert universal_critique_yaml_enabled_bucket_caption({'universal_critique_yaml_present': True, 'load_error': 'bad yaml'}) is None

def test_universal_critique_yaml_stage_keys_caption() -> None:
    cap = universal_critique_yaml_stage_keys_caption({'universal_critique_yaml_present': True, 'universal_critique_yaml_top_level_keys': ['test_writer', 'implementation', 'planner']})
    assert cap is not None
    assert 'implementation' in cap
    assert 'planner' in cap
    keys = [f'stage_{i}' for i in range(8)]
    cap_trunc = universal_critique_yaml_stage_keys_caption({'universal_critique_yaml_present': True, 'universal_critique_yaml_top_level_keys': keys})
    assert cap_trunc is not None
    assert '+2 more' in cap_trunc
    assert universal_critique_yaml_stage_keys_caption(None) is None
    assert universal_critique_yaml_stage_keys_caption({'universal_critique_yaml_present': False}) is None
    assert universal_critique_yaml_stage_keys_caption({'universal_critique_yaml_present': True, 'load_error': 'bad yaml'}) is None

def test_universal_critique_enabled_stages_caption() -> None:
    cap = universal_critique_enabled_stages_caption({'universal_critique_yaml_present': True, 'universal_critique_yaml_top_level_enabled_true_count': 2, 'universal_critique_yaml_top_level_enabled_false_count': 1, 'universal_critique_yaml_top_level_enabled_unset_mapping_count': 0, 'universal_critique_yaml_top_level_keys': ['implementation', 'planner', 'test_writer']})
    assert cap is not None
    assert 'enabled: true' in cap
    assert 'enabled: false' in cap
    assert universal_critique_enabled_stages_caption(None) is None
    assert universal_critique_enabled_stages_caption({'universal_critique_yaml_present': False}) is None

def test_explainer_enabled_true_counts_two_subtrees(tmp_path: Path) -> None:
    (tmp_path / 'configs' / 'workflows').mkdir(parents=True)
    (tmp_path / 'configs' / 'workflows' / 'two_en.yaml').write_text('version: 1\nuniversal_critique:\n  implementation:\n    enabled: true\n    llm: false\n  test_writer:\n    enabled: true\n    llm: false\n    stub: true\n', encoding='utf-8')
    out = universal_critique_workflow_explainer_payload(tmp_path, workflow_profile='two_en')
...
def test_explainer_yaml_top_level_keys_empty_when_block_absent(tmp_path: Path) -> None:
    (tmp_path / 'configs' / 'workflows').mkdir(parents=True)
    (tmp_path / 'configs' / 'workflows' / 'bare.yaml').write_text('version: 1\n', encoding='utf-8')
...
def test_universal_critique_env_override_summary_caption() -> None:
    no_delta = universal_critique_env_override_summary_caption({'yaml_only': {'tw_enabled': True}, 'effective_with_env': {'tw_enabled': True}})
    assert no_delta is not None
    assert 'no env overrides' in no_delta
    with_delta = universal_critique_env_override_summary_caption({'yaml_only': {'tw_enabled': True, 'pll_enabled': False}, 'effective_with_env': {'tw_enabled': False, 'pll_enabled': False}})
...
def test_workflow_vs_timeline_rows_no_timeline() -> None:
    rows = universal_critique_workflow_vs_timeline_rows({'universal_critique_yaml_top_level_enabled_true_count': 1}, None)
...
def test_workflow_vs_timeline_rows_stage_count_mismatch() -> None:
    rows = universal_critique_workflow_vs_timeline_rows({'universal_critique_yaml_top_level_enabled_true_count': 1}, {'stage_count': 3, 'fail_count': 1})
...
def test_workflow_vs_timeline_rows_with_full_timeline_paste_resolution() -> None:
    wall = {'run_id': '00000000-0000-4000-8000-000000000002', 'events': [], 'universal_critique': {'stage_count': 1, 'fail_count': 0, 'stages': []}}
    tl = universal_critique_snapshot_from_compare_paste(wall)
    rows = universal_critique_workflow_vs_timeline_rows({'universal_critique_yaml_top_level_enabled_true_count': 1}, tl)
    assert rows[0]['timeline_universal_critique'] == '1'
    assert 'stage_count matches' in rows[2]['timeline_universal_critique']

def test_universal_critique_workflow_explainer_operator_metrics() -> None:
    payload = {'universal_critique_yaml_present': True, 'universal_critique_yaml_top_level_keys': ['a', 'b'], 'universal_critique_yaml_top_level_enabled_true_count': 1, 'universal_critique_yaml_top_level_enabled_false_count': 0, 'universal_critique_yaml_top_level_enabled_unset_mapping_count': 1, 'universal_critique_yaml_top_level_mapping_child_count': 2, 'universal_critique_yaml_top_level_scalar_leaf_count': 1, 'yaml_only': {'default_enabled': True}}
...
def test_universal_critique_workflow_explainer_operator_metrics_list_child_count() -> None:
    payload = {'universal_critique_yaml_present': True, 'universal_critique_yaml_top_level_list_child_count': 2}
    m = universal_critique_workflow_explainer_operator_metrics(payload)
...
def test_universal_critique_workflow_explainer_operator_metrics_effective_panels() -> None:
    payload = {'universal_critique_yaml_present': True, 'universal_critique_yaml_top_level_keys': ['fw', 'mi'], 'effective_with_env': {'unanimous_gate_enforce': True, 'fw_enabled': True, 'mi_enabled': False}}
    m = universal_critique_workflow_explainer_operator_metrics(payload)
