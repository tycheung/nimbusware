...
def test_explainer_new_stage_env_overrides_yaml(
    repo_uc_stub: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
...
def test_universal_critique_yaml_top_level_mapping_child_count_caption(repo_uc_stub: Path) -> None:
    ...
    assert (
        universal_critique_yaml_top_level_mapping_child_count_caption({"load_error": "bad"}) is None
    )
...
def test_universal_critique_default_enabled_caption() -> None:
    ...
    payload = universal_critique_workflow_explainer_payload(
        root, workflow_profile="universal_critique_on"
    )
...
def test_universal_critique_yaml_enabled_bucket_caption() -> None:
    ...
    assert (
        universal_critique_yaml_enabled_bucket_caption({"universal_critique_yaml_present": False})
        is None
    )
    assert (
        universal_critique_yaml_enabled_bucket_caption(
            {"universal_critique_yaml_present": True, "load_error": "bad yaml"}
        )
        is None
    )
...
def test_universal_critique_yaml_stage_keys_caption() -> None:
    cap = universal_critique_yaml_stage_keys_caption(
        {
            "universal_critique_yaml_present": True,
            "universal_critique_yaml_top_level_keys": ["test_writer", "implementation", "planner"],
        }
    )
    assert cap is not None
    ...
    cap_trunc = universal_critique_yaml_stage_keys_caption(
        {"universal_critique_yaml_present": True, "universal_critique_yaml_top_level_keys": keys}
    )
    assert cap_trunc is not None
    assert "+2 more" in cap_trunc
    assert universal_critique_yaml_stage_keys_caption(None) is None
    assert (
        universal_critique_yaml_stage_keys_caption({"universal_critique_yaml_present": False})
        is None
    )
    assert (
        universal_critique_yaml_stage_keys_caption(
...
def test_universal_critique_enabled_stages_caption() -> None:
    ...
    assert (
        universal_critique_enabled_stages_caption({"universal_critique_yaml_present": False})
        is None
    )
...
def test_universal_critique_env_override_summary_caption() -> None:
    no_delta = universal_critique_env_override_summary_caption(
        {"yaml_only": {"tw_enabled": True}, "effective_with_env": {"tw_enabled": True}}
    )
...
def test_workflow_vs_timeline_rows_no_timeline() -> None:
    rows = universal_critique_workflow_vs_timeline_rows(
        {"universal_critique_yaml_top_level_enabled_true_count": 1}, None
    )
...
def test_workflow_vs_timeline_rows_with_full_timeline_paste_resolution() -> None:
    ...
    rows = universal_critique_workflow_vs_timeline_rows(
        {"universal_critique_yaml_top_level_enabled_true_count": 1}, tl
    )
