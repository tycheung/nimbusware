...
def test_stack_http_request_retries_connect_error() -> None:
    ...
    with patch("e2e.harness.stack.httpx.request", side_effect=[httpx.ConnectError("refused"), response]) as req:
        with patch("e2e.harness.stack.time.sleep") as sleep:
            out = stack_http_request("GET", "http://127.0.0.1:9/probe", retries=2, retry_delay_sec=0.01)
    ...
    with patch(
        ...
        with patch("e2e.harness.stack.time.sleep"):
            with pytest.raises(httpx.ConnectError):
                stack_http_request("GET", "http://127.0.0.1:9/probe", retries=2, retry_delay_sec=0.01)
