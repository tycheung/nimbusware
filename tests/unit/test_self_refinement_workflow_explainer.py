...
def test_self_refinement_policy_yaml_file_bytes_caption(repo_sr_policy_and_workflow: Path) -> None:
    out = self_refinement_workflow_explainer_payload(
        repo_sr_policy_and_workflow, workflow_profile="on"
    )
...
def test_self_refinement_merged_description_preview_caption() -> None:
    cap = self_refinement_merged_description_preview_caption(
        {"merged_description_preview": "override desc", "merged_description_len": 13}
    )
    assert cap is not None
    ...
    cap_trunc = self_refinement_merged_description_preview_caption(
        {"merged_description_preview": long_text, "merged_description_len": 150}, max_chars=120
    )
    ...
    assert (
        self_refinement_merged_description_preview_caption({"merged_description_preview": ""})
        is None
    )
...
def test_self_refinement_would_emit_marker_caption() -> None:
    ...
    cap_env_off = self_refinement_would_emit_marker_caption(
        {"would_emit_self_refinement_marker": True, "would_emit_marker_after_env": False}
    )
    assert cap_env_off is not None
    assert "kill-switch" in cap_env_off
    cap_off = self_refinement_would_emit_marker_caption(
        {"would_emit_self_refinement_marker": False, "would_emit_marker_after_env": False}
    )
    assert cap_off is not None
    assert "would not emit" in cap_off
...
def repo_sr_policy_and_workflow(tmp_path: Path) -> Path:
    ...
    (tmp_path / "configs" / "self_refinement" / "policy.yaml").write_text(
        _POLICY_AND_WORKFLOW_POLICY_TEXT, encoding="utf-8"
    )
    (tmp_path / "configs" / "workflows").mkdir(parents=True)
...
def test_marker_merge_vs_timeline_rows_no_timeline() -> None:
    rows = self_refinement_marker_merge_vs_timeline_rows(
        {"would_emit_self_refinement_marker": True, "would_emit_marker_after_env": True}, None
    )
...
def test_marker_merge_vs_timeline_version_mismatch() -> None:
    rows = self_refinement_marker_merge_vs_timeline_rows(
        {"merged_version": 1, "merged_description_len": 0}, {"version": 2}
    )
...
def test_marker_merge_vs_timeline_from_explainer_payload(repo_sr_policy_and_workflow: Path) -> None:
    ...
    rows = self_refinement_marker_merge_vs_timeline_rows(
        out["marker_merge"], {"version": 9, "description": "x"}
    )
    assert rows[4]["explainer_marker_merge"] == "match"
...
def test_marker_merge_vs_timeline_marker_count_from_timeline() -> None:
    rows = self_refinement_marker_merge_vs_timeline_rows(
        {"merged_version": 1, "merged_description_len": 0}, {"version": 1, "marker_count": 3}
    )
...
def test_marker_merge_vs_timeline_marker_count_absent_in_paste() -> None:
    rows = self_refinement_marker_merge_vs_timeline_rows(
        {"merged_version": 1, "merged_description_len": 0}, {"version": 1}
    )
    assert rows[2]["timeline_self_refinement"] == "—"
