...
def test_workflow_suppress_matrix_across_unpinned_maybe_callers_5_axis() -> None:
    ...
    with (
        patch(
            "orchestrator._pipeline.escalation.load_anti_deadlock_settings",
            return_value=(True, 0, 0),
        ),
        patch(
            "orchestrator._pipeline.escalation.should_emit_anti_deadlock_escalation",
            return_value=True,
        ),
    ):
    ...
    with patch(
        "orchestrator._pipeline.escalation.load_escalate_after_cumulative_gate_failures",
        return_value=2,
    ):
    ...
    _append_finding_row(mem_b3, rid_b3_default)
    with patch(
        "orchestrator._pipeline.escalation.load_auto_escalate_after_cumulative_findings",
        return_value=2,
    ):
    ...
    _append_finding_row(mem_b4, rid_b4)
    with patch(
        "orchestrator._pipeline.escalation.load_notice_escalate_at_cumulative_findings",
        return_value=2,
    ):
    ...
    with patch(
        "orchestrator._pipeline.escalation.load_escalate_on_first_verifier_failure",
        return_value=True,
    ):
