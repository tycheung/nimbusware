...
_GUARDS: tuple[tuple[str, int, frozenset[str]], ...] = (
    (
        ...
        frozenset(
            {
                "settings_catalog_extended/core_legacy_runtime.py",
            }
        ),
    ),
    (
...
def test_env_agent_module_size_limits() -> None:
    over_limit: list[str] = []
...
def test_env_agent_module_size_allowlists_are_current() -> None:
    stale: list[str] = []
    for rel_root, limit, allowlist in _GUARDS:
        root = _REPO / rel_root
        still_large = {
    ...
    assert not stale, "Update env/agent module size allowlists:\n" + "\n".join(stale)
