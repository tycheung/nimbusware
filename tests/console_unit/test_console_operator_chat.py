...
def test_natural_language_classify_suggestion(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "console.operator_chat_core.chat_svc.classify_intent",
        lambda _msg: resp,
    )
    state = ChatState()
...
def test_run_auto_uses_suggested_profile(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "console.operator_chat_core.chat_svc.create_run",
...
def test_steer_prefix_enqueues_on_active_run(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "console.operator_chat_core.chat_svc.enqueue_interjection",
