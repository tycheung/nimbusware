...
4. **Schema** — `kubectl apply -f schema-job.yaml`; wait for Job `Complete` (runs `scripts/database/apply_event_store.sh` / greenfield `postgres.sql`).
...
| `schema-job.yaml` | One-shot schema apply via `scripts/database/apply_event_store.sh` |
...
Worker / Redis fleet dispatch is also documented in `scripts/runbooks/run_dispatch_fleet_runbook.md`.
...
| Weekly dev-env soak | `scripts/ops/run_dev_env_weekly_soak.py` (CI job `dev-env-weekly-soak`) |
