...
**P14–P17 operator surfaces (see [../README.md](../README.md)):** TestPyPI bootstrap publish (`.github/workflows/publish_bootstrap.yml`); live theater SSE with named `event: theater` payloads (`headline`/`body_md`) in API + Maker (`tests/api/test_api_theater_stream.py`, `maker_chat_theater.spec.ts`); Progress memory-influence table + council ribbon (`maker_memory_influence.spec.ts`, `maker_council_ribbon.spec.ts`); industry critic packs with `review_rules` and `GET /v1/config/critic-packs/{id}/workflows`; Admin parity wiring (`tests/web/parity_admin_wiring.yaml`, 100% of admin `web: true` rows); Progress parity wiring (`tests/web/parity_progress_wiring.yaml`); PUT-preview launch-test journeys for seven SPA framework packs plus static HTML with keyboard/mouse fidelity in the PR framework gate (`playwright` dev dependency); asyncio-safe sync Playwright via `playwright_sync.py`; isolated bootstrap wheel install smoke including fresh-venv subprocess (`run_bootstrap_ci_gate.py`); publish workflow CI guard; persistent Playwright session cleanup in `browser_controller.py` + pytest autouse fixture; `ci_check.sh` extension compile parity with PowerShell.
