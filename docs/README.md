...
Context-efficiency APIs (see [../README.md](../README.md) § Context efficiency): `GET /v1/runs/{id}/context_budget`, `POST /v1/runs/{id}/compact` (scopes: `all`, `last_n`, `source_refs`), `POST /v1/runs/{id}/compactions/{compaction_id}/revert`, `POST /v1/runs/{id}/replay-from`, `GET/POST /v1/projects/{id}/context-artifacts`, `POST .../context-artifacts/{artifact_id}/bridge-memory` (optional `NIMBUSWARE_CONTEXT_ARTIFACT_FAISS_REBUILD=1`). **Workflow:** `micro_slice` enables `slice.e2e` by default; use `micro_slice_web` for web-first HTML globs. **Factory:** `campaign_factory_t3` profile, golden manifest, weekly CI matrix; live Playwright trace + console/network in PUT E2E `evidence.zip`; ISM per-slice diff; optional `NIMBUSWARE_FACTORY_EXPLORATORY_CRAWL`; `GET /v1/runs/{id}/factory-evidence` + export (optional object-store mirror); Admin/Maker factory status panels. **Maker PWA:** offline `sw.js`, push subscribe + **campaign milestone send** when VAPID public+private keys are set.
