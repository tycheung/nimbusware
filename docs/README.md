...
**P14–P16 operator surfaces (see [../README.md](../README.md)):** TestPyPI bootstrap publish (`.github/workflows/publish_bootstrap.yml`); live theater SSE (`tests/api/test_api_theater_stream.py`); industry critic packs with `review_rules` and `GET /v1/config/critic-packs/{id}/workflows`; Admin parity wiring (`tests/web/parity_admin_wiring.yaml`, 100% of admin `web: true` rows); PUT-preview launch-test journeys for seven SPA framework packs with keyboard/mouse fidelity in the PR framework gate (`playwright` dev dependency); isolated bootstrap wheel install smoke (`run_bootstrap_ci_gate.py`); publish workflow CI guard; persistent Playwright session cleanup in `browser_controller.py` + pytest autouse fixture; `ci_check.sh` extension compile parity with PowerShell.
