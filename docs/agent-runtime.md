...
- **Enforcement depth** — second 0–10 axis for workspace verify/gate strictness; presets in `configs/enforcement/presets.yaml`, wired into `micro_slice_verify` and `slice.gate` via `enforcement_pipeline.py`; API `GET/PUT /v1/runs/{id}/enforcement`; Maker Progress dual-slider (ADR [026](adr/026-enforcement-depth-slider.md))
