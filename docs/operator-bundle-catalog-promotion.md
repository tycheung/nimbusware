...
1. **Code Researcher** completes `research.pattern.indexed` and calls `write_catalog_candidate()` (see `packages/research/stages.py`).
2. **Stitcher** on successful `stitch.applied` writes a catalog candidate via `write_stitch_catalog_candidate()` (see `packages/research/stages_stitch.py`).
