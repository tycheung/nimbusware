...
Accepted (2026-06); amended (2026-07) — progressive compaction stack
...
Agent JIT loops also accumulate tool output and conversation history within a single slice.

## Decision

### Campaign handoff compaction (L3)
...
Compaction is layered; lower levels run inside a slice, L3 runs across campaign handoffs:

| Level | Trigger | Module |
|-------|---------|--------|
| **L1 offload** | Tool result exceeds `NIMBUSWARE_TOOL_OFFLOAD_CHARS` | `agent_core/tool_output_offload.py` |
| **L0 microcompact** | JIT loop token estimate exceeds threshold | `agent_tools/agent_loop.py` |
| **L3 campaign** | Handoff + campaign budget | `orchestrator/context_compaction.py` |
| **L4 full** | Operator `POST /v1/runs/{id}/compact` or `NIMBUSWARE_AGENT_COMPACT=full` | `agent_core/agent_full_compact.py` |

L0–L1 preserve raw tool events in the store; only prompt-facing history is trimmed.
L4 is operator-initiated and reversible via compaction revert routes.
...
- Slice implement loops stay bounded without losing audit events in the event store.
