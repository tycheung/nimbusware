...
Timeline summaries and field metadata were duplicated between `nimbusware_api.read_models` and Streamlit display modules. Divergence caused console/API parity bugs and made operator exports hard to keep consistent.

## Decision

Introduce `nimbusware_projections` with:

- `builders/` — pure functions over normalized event dicts (no HTTP or Streamlit)
