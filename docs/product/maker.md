...
- **Stack manifest:** confirmed manifest freezes surfaces (`api`, `web`) and stacks (`fastapi_python`, `react_vite`, …) from `configs/stacks/`; each stack entry may declare `max_files` / `max_loc` diff budgets merged with workflow and fleet caps; backlog and slice dispatch are surface-aware; Plan tab shows per-slice surface badges and `slice.contract` gate status
