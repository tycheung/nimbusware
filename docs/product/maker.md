...
- **Enforcement depth slider** — `GET/PUT /v1/runs/{id}/enforcement`; presets in `configs/enforcement/presets.yaml`; saved profiles in `configs/enforcement/user_profiles.yaml`; level 10 = workspace CI parity + terminal `enforcement.gate` (ADR [026](../adr/026-enforcement-depth-slider.md)). Progress tab shows `enforcement_status` chip; Build/Campaign start honor Settings default profile. Settings can save new strictness profiles (level + name).
- **Standards profile ribbon** — facade/bundle picker with save-to-profile; auto defaults from `configs/standards/preset_defaults.yaml` (language + enforcement level) when no custom overlay; `GET /v1/standards/presets/{id}/defaults`; Settings default standards profile optional at run start.
