...
.DESCRIPTION
  Runs ``poetry install --with faiss`` then ``scripts/faiss/build_bundle_faiss_index.py`` with
...
poetry run python scripts/faiss/build_bundle_faiss_index.py --repo-root "$root"
