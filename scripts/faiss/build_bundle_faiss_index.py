...
``poetry run python`` ``scripts/faiss/build_bundle_faiss_index.py`` from repo root.
Local wrappers (same commands): ``scripts/faiss/build_bundle_faiss_index.ps1`` (Windows) and
``scripts/faiss/build_bundle_faiss_index.sh`` (POSIX).
"""
...
def build_bundle_faiss_index(*, catalog_path: Path, out_dir: Path) -> int:
    ...
    from extensions import BundleCatalog
