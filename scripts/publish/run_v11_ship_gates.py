...
def main() -> int:
    ...
    steps: list[tuple[str, list[str], bool]] = [
        ...
        (
            ...
            [py, "scripts/publish/publish_vscode_extension.py"],
            False,
        ),
    ]
        ...
        steps.extend(
            [
                (
                    ...
                    [
                        py,
                        "scripts/benchmarks/measure_intent_to_patch.py",
                        "--via-chat",
                        "--json",
    ...
    if args.publish_testpypi:
        if not _run(
            ...
            [py, "scripts/publish/publish_bootstrap_release.py", "--testpypi", "--skip-gate"],
            optional=False,
        ):
            failed += 1
    ...
    if args.publish_pypi:
        if not _run(
            ...
            [py, "scripts/publish/publish_bootstrap_release.py", "--pypi", "--skip-gate"],
            optional=False,
        ):
            failed += 1
    ...
    if args.publish_vscode:
        if not _run(
            ...
            [py, "scripts/publish/publish_vscode_extension.py", "--publish"],
            optional=False,
        ):
            failed += 1
