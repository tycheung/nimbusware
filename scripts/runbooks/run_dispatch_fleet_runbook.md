...
```bash
poetry run python scripts/ops/run_dispatch_worker.py
```

Bounded smoke (exits when idle):

```bash
poetry run python scripts/ops/run_dispatch_worker.py --max-idle-loops 50 --idle-sleep-seconds 0.1
```
...
3. Verify worker heartbeat file or pod logs show monotonic `processed` counts (`scripts/ops/run_dispatch_worker.py --heartbeat-path /tmp/worker.json`).
