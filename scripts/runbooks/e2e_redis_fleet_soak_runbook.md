...
Weekly **redis-fleet-soak** job in [`.github/workflows/slow_tests.yml`](../.github/workflows/slow_tests.yml) runs `scripts/ops/run_redis_fleet_soak_ci.py` against one or more Redis URLs (`NIMBUSWARE_REDIS_FLEET_URLS`, default `NIMBUSWARE_REDIS_URL`). CI uses two Redis service containers on ports 6379 and 6380. Local runs skip gracefully when Redis is unreachable.
