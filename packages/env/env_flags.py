...
def env_tri_state(name: str) -> str | None:
    from env.settings_resolve import resolve_tri_state
...
def api_host(default: str = "0.0.0.0") -> str:
...
def api_port(default: int = 8000) -> int:
...
def api_base_url(*, default: str = "http://127.0.0.1:8000/v1") -> str:
    ...
    if host in ("0.0.0.0", "::"):
        host = "127.0.0.1"
    return f"http://{host}:{api_port()}/v1"


def config_from_files_enabled() -> bool:
...
def nimbusware_preflight_latency_sample_count(default: int = 1) -> int:
    from env.settings_resolve import resolve_int as _ri
...
def memory_excerpt_max_chars(default: int = 4000) -> int:
...
from env.env_flags_infra import *  # noqa: E402, F403
