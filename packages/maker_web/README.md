...
| Review | `review.js` + `review_git_ui.js` (git/PR + deploy audit), `review_*_ui.js`, `deploy_cockpit.js` | maker pending, deploy cockpit (validate + approve), deploy audit timeline, git/PR/CI status, scorecard |
...
| `ui_shared` via `../../../ui_shared/` | api-core, formatters, launch-scorecard, theater-dom |

Mount: [`packages/api/routes/maker_web.py`](../api/routes/maker_web.py).
