...
```bash
cd packages/admin_ui
npm ci
npm run build
```

Local `scripts/ci/ci_check.ps1` runs `npm run build` then vitest before Playwright.

Deep links such as `/v1/admin/app/runs/{id}` are served via SPA fallback (`AdminSPAStaticFiles` in `api.routes.admin_web`); the Preact router strips the `/v1/admin/app` prefix client-side.
