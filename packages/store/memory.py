...
from store.allowed_types import assert_event_type_registered
from store.protocol import event_row_from_serialized, serialized_event_from_row
from store.tenant_scope import store_tenant_id
...
class InMemoryEventStore:
    ...
    def append(self, event: NimbuswareEventUnion) -> int:
    ...
    def _replay_list_status(self, run_id: UUID) -> str:
        """Match ``orchestrator.read_models.build_run_summary`` status for non-empty runs."""
        rows = [r for r in self._scoped_rows() if r["run_id"] == run_id]
        rows.sort(key=lambda r: int(r["store_seq"]))
        if not rows:
            return "unknown"
