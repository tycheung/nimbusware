...
from agent_tools.risk_caps import (
    AgentRiskCaps,
    resolve_agent_risk_caps,
)
from agent_tools.tool_registry import is_agent_tool_enabled
...
def _execute_step(
    ...
    *,
    ...
    if step.tool == "write_ui_flow":
        from agent_tools.tools import tool_write_ui_flow
    ...
    if step.tool == "run_ui_regression":
        from agent_tools.tools import tool_run_ui_regression
...
def execute_slice_implement_agent(
    ...
    *,
    ...
    if use_llm and llm_base_url is not None and llm_model_id is not None:
        ...
        from env.env_flags import nimbusware_agent_jit_loop_enabled

        if nimbusware_agent_jit_loop_enabled():
            from agent_tools.agent_loop import run as run_agent_loop
