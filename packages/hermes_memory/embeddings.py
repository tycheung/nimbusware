...
_DETERMINISTIC_MODEL_ID = "hermes-memory-deterministic-v1"
...
def _default_ollama_embedding_model() -> str:
    env = os.environ.get("HERMES_MEMORY_EMBEDDING_MODEL", "").strip()
    if env:
        return env
...
def _ollama_runtime_from_env() -> tuple[str, str]:
    base = os.environ.get("HERMES_OLLAMA_BASE_URL", "").strip()
    if not base:
