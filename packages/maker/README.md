...
Server-side **maker approval** state machine and slice workflow helpers. The operator UI is the Alpine web app in [`maker_web`](../maker_web/README.md) at `/v1/maker/app/`.

## Entry

- `packages/maker/cli.py` — `poetry run nimbusware-maker` (starts API + web shell when configured)
...
- API routes under `api/routes/runs/maker_*.py` implement plan/slice approval, pending state, git status, and workspace revert

## Web tabs (via `maker_web`)
...
| `slice_engine.py` | **Sole** module-level import site for `orchestrator` slice execution |
...
HTTP to the API uses `NIMBUSWARE_API_BASE` and optional `NIMBUSWARE_API_KEY` / admin token headers from `client`.
