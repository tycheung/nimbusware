...
- Must **not** import `nimbusware_orchestrator` at module level (enforced by `tests/unit/test_import_graph.py`).
- Prefer pure helpers; side effects belong in orchestrator or API routes.

## Consumers

`nimbusware_orchestrator` (workflow merge, persona shelves), `nimbusware_console` (config tooling, catalog panels), `nimbusware_api` (admin config routes).
