...
def load_registry_from_postgres(conninfo: str) -> RoleRegistry:
    """Load taxonomy keys from ``hermes_roles_registry`` (see ``schema/postgres.sql``)."""
    with psycopg.connect(conninfo) as conn:
        with conn.cursor() as cur:
            cur.execute(
                """
                SELECT taxonomy_key, role_id
                FROM hermes_roles_registry
    ...
    if not rows:
        msg = (
            "hermes_roles_registry is empty; apply "
            "packages/hermes_store/schema/postgres.sql (bootstrap to an empty database)"
        )
