...
def slice_commit_message(plan: SlicePlan) -> str:
    ...
    return f"[hermes] {plan.slice_id}: {msg}"


def run_complete_commit_message(run_id: str | UUID, *, slice_count: int) -> str:
    return f"[hermes] run {run_id} complete ({slice_count} slice(s) passed gates)"
...
def maybe_open_gh_pr(
    ...
    *,
    ...
    if not (workspace / ".git").is_dir():
        return {"status": "skipped", "reason": "not_git_repo"}
    branch = run_branch_name(run_id)
    pr_title = title or f"Hermes run {run_id}"
    pr_body = body or f"Automated PR for Hermes run `{run_id}`."
    try:
        proc = subprocess.run(
            [
                "gh",
