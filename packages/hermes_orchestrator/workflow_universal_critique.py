"""Workflow YAML knobs for universal post-verify critique panels (plan §14 #16).

When ``HERMES_<KNOB>`` is unset, the frozen workflow YAML ``universal_critique`` block
applies for that knob. Non-empty env values override YAML (truthy vs non-truthy
same semantics as existing critique env gates).

Optional critique gate FAIL → ``finding.created`` uses
``emit_finding_on_gate_fail`` per stage and env
...
Optional **hard-block** (skip integrator / agent-evaluator / self-refinement tail after verify)
uses ``hard_block_on_gate_fail`` per stage and env
...
_UC_PANEL_KILL_SWITCH_KEYS = (
    ...
    "HERMES_ENABLE_FRONTEND_WRITER_CRITIQUE",
    "HERMES_ENABLE_MODULE_INTEGRATOR_CRITIQUE",
)
...
def effective_universal_critique(
    ...
    *,
    ...
    return EffectiveUniversalCritique(
        impl_llm=env_over_yaml("HERMES_IMPLEMENTATION_CRITIQUE_LLM", wf.impl_llm),
        impl_stub=env_over_yaml("HERMES_STUB_IMPLEMENTATION_CRITICS", wf.impl_stub),
        ...
        tw_enabled=env_over_yaml("HERMES_ENABLE_TEST_WRITER_CRITIQUE", wf.tw_enabled),
        tw_llm=env_over_yaml("HERMES_TEST_WRITER_CRITIQUE_LLM", wf.tw_llm),
        tw_stub=env_over_yaml("HERMES_STUB_TEST_WRITER_CRITICS", wf.tw_stub),
        ...
        pll_enabled=env_over_yaml("HERMES_ENABLE_PLANNER_CRITIQUE", wf.pll_enabled),
        pll_llm=env_over_yaml("HERMES_PLANNER_CRITIQUE_LLM", wf.pll_llm),
        pll_stub=env_over_yaml("HERMES_STUB_PLANNER_CRITICS", wf.pll_stub),
        ...
        fw_enabled=env_over_yaml("HERMES_ENABLE_FRONTEND_WRITER_CRITIQUE", wf.fw_enabled),
        fw_llm=env_over_yaml("HERMES_FRONTEND_WRITER_CRITIQUE_LLM", wf.fw_llm),
        fw_stub=env_over_yaml("HERMES_STUB_FRONTEND_WRITER_CRITICS", wf.fw_stub),
        ...
        mi_enabled=env_over_yaml("HERMES_ENABLE_MODULE_INTEGRATOR_CRITIQUE", wf.mi_enabled),
        mi_llm=env_over_yaml("HERMES_MODULE_INTEGRATOR_CRITIQUE_LLM", wf.mi_llm),
        mi_stub=env_over_yaml("HERMES_STUB_MODULE_INTEGRATOR_CRITICS", wf.mi_stub),
