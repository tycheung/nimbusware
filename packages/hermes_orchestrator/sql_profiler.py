"""SQL query profiler: runtime session counter + static hotspot scan (PZ-6)."""
...
_sql_query_count_var: ContextVar[int] = ContextVar("hermes_sql_query_count", default=0)
...
def check_runtime_sql_budget() -> tuple[int, str, dict[str, Any]]:
    """Compare session counter and/or ``HERMES_TEST_SQL_QUERY_COUNT`` to limit."""
    max_raw = os.environ.get("HERMES_SQL_QUERY_COUNT_MAX", "").strip()
    if not max_raw:
        return 0, "sql profiler runtime: skipped (HERMES_SQL_QUERY_COUNT_MAX unset)\n", {}
    try:
        limit = max(1, int(max_raw))
    except ValueError:
        return 0, "sql profiler runtime: invalid HERMES_SQL_QUERY_COUNT_MAX\n", {}

    session_count = current_sql_query_count()
    env_raw = os.environ.get("HERMES_TEST_SQL_QUERY_COUNT", "").strip()
    count: int | None = session_count if session_count > 0 else None
    if env_raw:
        try:
            count = int(env_raw)
        except ValueError:
            return 1, "sql profiler runtime: invalid HERMES_TEST_SQL_QUERY_COUNT\n", {}
