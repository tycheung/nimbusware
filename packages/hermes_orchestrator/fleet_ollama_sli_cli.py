"""Fleet sustained Ollama p95 export: ``poetry run hermes-fleet-ollama-sli``."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--base-url",
        default="",
        help="Ollama base URL (default: HERMES_FLEET_OLLAMA_SLI_BASE_URL or localhost:11434)",
    )
    ...
    if args.require_enterprise and edition() != ENTERPRISE_EDITION:
        print(
            f"hermes-fleet-ollama-sli requires {ENV_EDITION}={ENTERPRISE_EDITION} "
            f"(current: {edition()!r})",
            file=sys.stderr,
        )
        return 2
