...
from hermes_orchestrator.preflight_histogram import build_histogram, nearest_rank_p95
...
def sustained_probe_config() -> dict[str, Any]:
    return {
        "samples": _int_env("HERMES_FLEET_OLLAMA_SLI_SAMPLES", 60, minimum=1, maximum=500),
        ...
        "timeout_seconds": _float_env(
            "HERMES_FLEET_OLLAMA_SLI_TIMEOUT_SEC",
            30.0,
            minimum=1.0,
        ),
    }
...
def export_path() -> Path:
    raw = os.environ.get("HERMES_FLEET_OLLAMA_SLI_EXPORT_PATH", "").strip()
...
def resolve_runtime_url(*, base_url: str | None = None) -> str:
    ...
    env_url = os.environ.get("HERMES_FLEET_OLLAMA_SLI_BASE_URL", "").strip()
    if env_url:
...
def resolve_health_path() -> str:
    raw = os.environ.get("HERMES_FLEET_OLLAMA_SLI_HEALTH_PATH", "").strip()
