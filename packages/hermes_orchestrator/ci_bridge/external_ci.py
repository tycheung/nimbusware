...
def _github_repo() -> tuple[str, str] | None:
    raw = os.environ.get("HERMES_CI_GITHUB_REPO", "").strip()
    if "/" not in raw:
        return None
...
def notify_gate_decision_external(
    *,
    ...
    owner, name = repo
    base = (timeline_base_url or os.environ.get("HERMES_TIMELINE_BASE_URL", "")).strip()
    link = f"{base.rstrip('/')}/v1/runs/{run_id}/timeline" if base else ""
    conclusion = "success" if str(verdict).upper() == "PASS" else "failure"
    sha = os.environ.get("HERMES_CI_HEAD_SHA", "0000000000000000000000000000000000000000")
    check_name = f"hermes/{stage_name}"
    ...
    body: dict[str, Any] = {
        ...
        "output": {
            "title": f"Hermes gate {stage_name}",
