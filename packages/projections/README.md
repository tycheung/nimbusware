...
Projections must not import `orchestrator` at module level (campaign/backlog parsers live in `agent_core.read.campaign`). Orchestrator must not import `api`. Enforced in `tests/unit/test_import_graph.py`.
