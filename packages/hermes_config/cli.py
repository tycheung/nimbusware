...
_TOOL_NAME = "hermes-config"


def _repo_root(arg: str | None) -> Path:
    if arg:
        return Path(arg).resolve()
    return Path(os.environ.get("HERMES_REPO_ROOT", ".")).resolve()


def _database_url() -> str:
    url = os.environ.get("HERMES_DATABASE_URL", "").strip()
    if not url:
        msg = "HERMES_DATABASE_URL is required for hermes-config"
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--repo-root",
        default=None,
        help="Repository root (default: $HERMES_REPO_ROOT or '.').",
    )
