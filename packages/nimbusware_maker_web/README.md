...
| Chat | `chat.js` + `chat_session_lifecycle.js`, `chat_shell_html.js`, `chat_run_card_ui.js`, `chat_collab_wiring.js`, `chat_model_drawer_ui.js`, `chat_invite_modal_ui.js`, `chat_mention_ui.js`, `chat_join.js`, `chat_*_ui.js` | `/chat/sessions`, classify/start, `@` routing, join discipline picker, collab roster |
...
| Review | `review.js` + `review_*_ui.js`, `deploy_cockpit.js` | maker pending, deploy cockpit (Terraform validate), scorecard |
| Progress | `static/js/tabs/progress.js` + `progress/*`, `deploy_cockpit.js` | theater SSE, deploy cockpit + Terraform validate, ribbons |
...
| `deploy_cockpit.js`, `chat_mention_ui.js`, `chat_join.js` | Deploy cockpit + Terraform validate; `@` discipline autocomplete + routing |
