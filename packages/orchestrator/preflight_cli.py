...
from orchestrator.pipeline import default_paths
from orchestrator.preflight import PreflightError, run_model_preflight
from orchestrator.preflight_histogram import (
    BUCKET_EDGES_MS,
    build_histogram,
    empty_histogram,
)
...
_SCHEMA_VERSION = 1
_TOOL_NAME = "nimbusware-preflight"
    ...
    *,
    ...
    try:
        if samples is not None:
            saved["NIMBUSWARE_PREFLIGHT_LATENCY_SAMPLES"] = os.environ.get(
                "NIMBUSWARE_PREFLIGHT_LATENCY_SAMPLES",
            )
            os.environ["NIMBUSWARE_PREFLIGHT_LATENCY_SAMPLES"] = str(samples)
        if json_probe:
            saved["NIMBUSWARE_PREFLIGHT_JSON_PROBE"] = os.environ.get(
                "NIMBUSWARE_PREFLIGHT_JSON_PROBE",
            )
            os.environ["NIMBUSWARE_PREFLIGHT_JSON_PROBE"] = "1"
        yield
    finally:
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog=_TOOL_NAME,
        description=(
            "Run the Nimbusware agent Ollama preflight probe ad-hoc and emit a JSON "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "$NIMBUSWARE_PREFLIGHT_LATENCY_SAMPLES for this invocation. "
    ...
    parser.add_argument(
        "--json-probe",
        action="store_true",
        help="Force $NIMBUSWARE_PREFLIGHT_JSON_PROBE=1 for this invocation.",
    )
    parser.add_argument(
        "--output",
        default="-",
