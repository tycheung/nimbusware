...
from orchestrator.llm_slice import execute_slice_plan_llm
    ...
    return parse_slice_plan(
        {
            ...
            "target_paths": [
                "packages/orchestrator/micro_slice.py",
                "packages/orchestrator/slice_gate.py",
            ],
...
def custom_agent_system_prompt(orch: RunOrchestrator, rows: list[dict[str, Any]]) -> str | None:
    from config.persist import load_custom_agent_registry
...
def plan_one_slice(
    ...
    *,
    ...
    from orchestrator.patch_context import patch_slice_plan_for_run
