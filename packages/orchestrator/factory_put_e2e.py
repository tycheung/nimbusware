...
from orchestrator.factory_completion import FactoryTier, evaluate_factory_gates
...
def run_put_e2e_for_factory_run(
    ...
    *,
    ...
    if store is not None and run_id is not None and preview.handle is not None:
        from orchestrator.put_runtime import emit_put_preview_started
    ...
    try:
        ...
        from orchestrator.ism_diff import (
            diff_ism_snapshots,
            load_ism_snapshot,
        ...
        if store is not None and run_id is not None and preview.handle is not None:
            from orchestrator.put_runtime import emit_put_preview_stopped
