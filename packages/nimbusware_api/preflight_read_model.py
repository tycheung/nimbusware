"""Shared preflight timeline projection (avoids circular imports between route modules)."""
...
def preflight_timeline_summary(events: list[dict[str, Any]]) -> dict[str, Any] | None:
    ...
    Follows the same ``latest wins`` convention as the other ``*_timeline_summary``
    helpers: walks the event list once and overwrites ``out`` on every match so
    the final value is the most recently appended preflight event. Returns
    ``None`` when no ``model.preflight.passed`` event is present (e.g. preflight
    was skipped via ``HERMES_SKIP_PREFLIGHT``).
    """
