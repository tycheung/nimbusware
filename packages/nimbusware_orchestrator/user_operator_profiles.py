...
def _safe_coding_user_path(repo_root: Path, user_id: str) -> Path:
    return repo_root / "configs" / "autopilot" / "users" / f"{user_id.strip()}_safe_coding.yaml"
...
def load_user_industry_critic_pack_ids(
    user_id: str,
    *,
    repo_root: Path | None = None,
) -> list[str]:
    ...
    if not uid:
        return []
    root = repo_root or find_repo_root()
    path = _safe_coding_user_path(root, uid)
    if not path.is_file():
        return []
    raw = yaml.safe_load(path.read_text(encoding="utf-8"))
    if not isinstance(raw, dict):
        return []
    packs = raw.get("industry_critic_pack_ids")
    if not isinstance(packs, list):
        return []
    return [str(p).strip() for p in packs if str(p).strip()]


def save_user_industry_critic_pack_ids(
    user_id: str,
    pack_ids: list[str],
    *,
    repo_root: Path | None = None,
    ...
    path = _safe_coding_user_path(root, uid)
    path.parent.mkdir(parents=True, exist_ok=True)
    cleaned = [str(p).strip() for p in pack_ids if str(p).strip()]
    path.write_text(
        yaml.safe_dump({"industry_critic_pack_ids": cleaned}, sort_keys=False),
        encoding="utf-8",
    )
    return {"user_id": uid, "industry_critic_pack_ids": cleaned}


def enrich_participants_with_profiles(
    ...
    *,
    ...
    for row in participants:
        ...
        if uid:
            profiles = load_user_operator_profiles(uid, repo_root=root)
            if profiles.get("autopilot_profile_id"):
                item["autopilot_profile_id"] = profiles["autopilot_profile_id"]
            if profiles.get("enforcement_profile_id"):
                item["enforcement_profile_id"] = profiles["enforcement_profile_id"]
        out.append(item)
    return out
