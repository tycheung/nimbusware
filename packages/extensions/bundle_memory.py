...
from extensions.bundle_memory_models import BundleOutcomeRecord, BundleSuccessStats
...
def bundle_memory_rank_weight() -> float:
    from env.env_flags import nimbusware_bundle_memory_rank_weight
    ...
    def append(self, record: BundleOutcomeRecord) -> None:
        with psycopg.connect(self._conninfo) as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
                    INSERT INTO nimbusware_bundle_outcome (
        ...
        with psycopg.connect(self._conninfo) as conn:
            with conn.cursor(row_factory=dict_row) as cur:
                cur.execute(
                    ...
                    FROM nimbusware_bundle_outcome
