...
def universal_critique_workflow_vs_timeline_rows(
    ...
    if tl is None:
        ...
        tl_stage_disp = no_tl if not isinstance(sc, int) or isinstance(sc, bool) else str(sc)
        fc = tl.get("fail_count")
        tl_fail_disp = no_tl if not isinstance(fc, int) or isinstance(fc, bool) else str(fc)
