...
def universal_critique_yaml_value_nonempty(value: Any) -> bool:
    if value is None:
        return False
...
def universal_critique_top_level_nonempty_count(uc: Mapping[str, Any]) -> int:
    return sum(1 for v in uc.values() if universal_critique_yaml_value_nonempty(v))
...
def universal_critique_top_level_scalar_leaf_count(uc: Mapping[str, Any]) -> int:
    return sum(1 for v in uc.values() if not isinstance(v, (dict, list)))


def universal_critique_top_level_list_child_count(uc: Mapping[str, Any]) -> int:
