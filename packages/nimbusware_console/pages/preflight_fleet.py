    ...
    with st.expander("Cross-run preflight trends (from last list)", expanded=False):
        ...
        if isinstance(_hist_saved, dict) and _ids_trend:
            ...
            if _pf_hist_export_json != "{}":
                _pf_hist_export_slug = preflight_history_metrics_export_download_filename_slug()
                ...
                st.download_button(
                    ...
                    file_name=(f"hermes_{_pf_hist_export_slug}_{_pf_hist_export_ts}.json"),
        ...
        if isinstance(_trend_rows, list) and _trend_rows:
            ...
            _pref_trend_metrics_slug = preflight_cross_run_operator_metrics_export_filename_slug()
            if _pref_trend_metric_rows:
                ...
                _pref_trend_metrics_json = preflight_cross_run_operator_metrics_export_json(
                    _pref_trend_metrics,
                )
                _pref_trend_metrics_csv = preflight_cross_run_operator_metrics_table_rows_csv(
                    _pref_trend_metric_rows,
                )
                (
                ...
                with _pref_trend_metrics_dl_json_col:
                    st.download_button(
                        label=("Download preflight cross-run operator metrics JSON"),
                ...
                with _pref_trend_metrics_dl_csv_col:
                    if _pref_trend_metrics_csv:
                        st.download_button(
                            label=("Download preflight cross-run operator metrics CSV"),
