...
def render_faiss_exports_panel(
    ...
    *,
    ...
    _faiss_ready_metrics_json = bundle_faiss_readiness_summary_operator_metrics_export_json(
        _faiss_sum_metrics
    )
    _faiss_ready_metrics_csv = bundle_faiss_readiness_summary_operator_metrics_table_rows_csv(
    ...
    with _faiss_ready_dl_json_col:
        st.download_button(
            ...
            file_name=(f"hermes_bundle_faiss_readiness_{_faiss_ready_slug}_{_faiss_ready_ts}.json"),
            mime="application/json",
    ...
    with _faiss_ready_dl_csv_col:
        if _faiss_ready_csv:
            st.download_button(
                ...
                file_name=(
                    f"hermes_bundle_faiss_readiness_{_faiss_ready_slug}_{_faiss_ready_ts}.csv"
                ),
                mime="text/csv; charset=utf-8",
    ...
    if _faiss_status_rows:
        ...
        with _faiss_status_dl_json_col:
            st.download_button(
                ...
                file_name=(
                    f"hermes_bundle_faiss_index_status_{_faiss_status_slug}_{_faiss_status_ts}.json"
                ),
                mime="application/json",
