...
def compute_work_queue_mode() -> str | None:
    from env.env_flags import compute_work_queue_mode

    return compute_work_queue_mode()
...
def get_work_unit_queue() -> WorkUnitQueuePort:
    ...
    if mode == "redis":
        from env.env_flags import nimbusware_redis_url
    ...
    if mode == "postgres":
        ...
        if not postgres_url:
            ...
            raise ValueError(msg)
        from compute.work_unit_postgres import PostgresWorkUnitQueue
