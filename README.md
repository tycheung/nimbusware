...
**Onboarding targets** (indicative on a mid-tier dev machine — see [docs/deploy/first-install-timing.md](docs/deploy/first-install-timing.md)): install → first gate pass **&lt; 15 min**; quick stub path → first gate pass **&lt; 3 min** after `poetry run nimbusware-run --quick`. **Intent→patch timing** is measured on the `tiny_python_app` fixture with stub implement (`scripts/benchmarks/measure_intent_to_patch.py`; chat path: `--via-chat`); results land in committed snapshots `benchmarks/latest_intent_to_patch.json` and `benchmarks/latest_classifier_acceptance.json` for Admin **Metrics**. Regenerate after local runs; PR CI validates snapshot targets via `scripts/ci/run_intent_to_patch_ci_gate.py` and `scripts/ci/run_classifier_acceptance_ci_gate.py`. Chat-turn store analytics: `GET /v1/platform/analytics/chat-turns`. **Chat tab** interleaves live theater and gate outcomes in the main thread (fork/branch panel for sibling paths). **Admin Policy compare** records gate pass-rate delta on **Metrics**.
...
- **Enterprise fleet (Admin)** — **Fleet** tab at `/v1/admin/app/fleet` (tenant switcher, fleet memory, Ollama SLI aggregate, worker health, hardware tiers with live **Rescan fleet hosts**, cross-tenant gate comparison with CSV export; requires `X-Nimbusware-Api-Key` in Admin sign-in)
...
| **Desktop / env** | `nimbusware_env` | Edition gate, **237-key** catalog + `env_flags` helpers, admin token guards, desktop launchers |
...
Environment toggles use the **`NIMBUSWARE_*`** prefix (**237** catalog keys). Resolve via [`env_flags.py`](packages/nimbusware_env/env_flags.py) helpers (`nimbusware_database_url`, `nimbusware_repo_root_path`, `nimbusware_api_base_url`, `nimbusware_ollama_base_url`, `nimbusware_api_port`, `nimbusware_github_token`, …) and [`docs/operator-settings.md`](docs/operator-settings.md) — not raw `os.environ` in feature code (`scripts/ci/audit_operator_env.py` enforces catalog coverage). See [`.env.example`](.env.example).
...
- **Parallel writers** — frontend writer validates or scaffolds web assets (`frontend_writer_stage.py`); backend/test writers run verifiers and optional LLM bodies; parallel dispatch via `asyncio.gather`
- **Bundle integrator** — catalog search, FAISS ranking, compatibility scoring, integrator gate (live adapter probe on gate metadata); optional GitHub Checks or GitLab commit status bridge on integrator, **slice.gate**, and **factory.gate** ([docs/deploy/external-ci-bridge.md](docs/deploy/external-ci-bridge.md))
...
- **Slice browser verify (`slice.e2e`)** — **on by default** (`slice.e2e.enabled: true` in [`configs/workflows/micro_slice.yaml`](configs/workflows/micro_slice.yaml)). Use [`configs/workflows/micro_slice_web.yaml`](configs/workflows/micro_slice_web.yaml) for the explicit **web-first** profile (HTML globs). Install Playwright (`poetry run playwright install`) or set `NIMBUSWARE_SLICE_E2E_COMMAND` to a custom shell command. If the runner or `tests/e2e` is missing, the stage **SKIP**s and the slice gate still passes. Default PR **unit** CI runs the `micro_slice_web` apply journey with a command that asserts `index.html` exists in the fixture workspace (see [`scripts/ci/ci_check.ps1`](scripts/ci/ci_check.ps1)); PR **web** job runs **55** Playwright tests across **41** spec files in [`tests/e2e/web`](tests/e2e/web) (including Chat patch/override/theater SSE, Admin hardware catalog + fleet hardware table + fleet rescan, Admin run-detail panels + audit export, Admin config/fleet/nav smoke, home intents, gate-fail actions, Settings hybrid routing presets, operator ribbons, memory influence, council ribbon, variant fitness, mobile theater parity). VS Code status extension compiles in `ci_check` (PowerShell and shell) when Node is installed ([`extensions/nimbusware-status/`](extensions/nimbusware-status/)).
- **Factory scaffold (v2 preview)** — PUT preview runtime (`put_runtime.py`), factory tiers T0–T3 (`factory_completion.py`, [`configs/factory/factory_tier_policy.yaml`](configs/factory/factory_tier_policy.yaml)), interaction surface map with optional runtime crawl (`interaction_surface_map.py`), PUT E2E flow runner (`put_e2e_runner.py`, [`configs/factory/flows/`](configs/factory/flows/) incl. `crm`, `static_site`, `contacts_api`, `todo_api`) with live Playwright trace on pass and fail when capture is enabled; PUT E2E failure evidence zip (`put_e2e_evidence.py`); automatic factory cadence on maintenance passes (`factory_cadence.py`, profiles [`campaign_factory_zero_touch.yaml`](configs/workflows/campaign_factory_zero_touch.yaml) and [`campaign_factory_t3.yaml`](configs/workflows/campaign_factory_t3.yaml)); weekly job [`scripts/benchmarks/run_factory_weekly_ci.py`](scripts/benchmarks/run_factory_weekly_ci.py) with multi-entry [`golden_factory_replay_manifest.json`](tests/fixtures/factory/golden_factory_replay_manifest.json) in [`.github/workflows/slow_tests.yml`](.github/workflows/slow_tests.yml) (`factory-weekly`); opt-in PR PUT E2E gate via `put-e2e` label ([`scripts/ci/run_put_e2e_ci_gate.py`](scripts/ci/run_put_e2e_ci_gate.py)); fleet remote Playwright pool via `NIMBUSWARE_FLEET_PLAYWRIGHT_WS_ENDPOINT` (`fleet_playwright.py`); factory evidence `GET /v1/runs/{id}/factory-evidence` and zip `.../factory-evidence/export` (`factory_evidence.py`).
...
- **Variable PUT launch testing (v2.4)** — resolve HTTP + UI flows per prompt/stack (`launch_flow_resolver.py`); catalog UI flows in [`configs/factory/ui_flows/`](configs/factory/ui_flows/) (e.g. `todo_api_ui`); Playwright locators via role/testid/label/text; full-stack sessions expose `api_base_url` + `frontend_base_url`; launch-test writer/critic stages on [`micro_slice_fullstack`](configs/workflows/micro_slice_fullstack.yaml) with ISM synthesis, optional LLM writer (`launch_test_llm.py`), and critique replan loop (`launch_test_stage.py`); nine JS framework packs in [`configs/launch_test/frameworks/`](configs/launch_test/frameworks/) (react/vue/angular/svelte/static/spa + next/nuxt/remix) with `writer_instructions` in `build_launch_test_writer_prompt()`; golden fixtures [`tests/fixtures/repos/tiny_todo_fullstack/`](tests/fixtures/repos/tiny_todo_fullstack/) and [`tests/fixtures/repos/tiny_spa_unknown/`](tests/fixtures/repos/tiny_spa_unknown/); journeys include fullstack launch, unknown-SPA synthesis, parametrized framework detect (`test_launch_framework_detect_journey.py`), PUT-preview plan/write/critique for all seven SPA packs plus static HTML (`test_launch_framework_put_preview_journey.py`, `test_launch_test_static_html_journey.py`), and write-replan UI+keyboard (`tests/e2e/journeys/`). Sync Playwright runs on a dedicated worker thread (`playwright_sync.py`) so UI flows and human-fidelity checks work under FastAPI/asyncio test clients. Live theater SSE is covered by `tests/api/test_api_theater_stream.py` and stack journey `test_theater_stream_journey.py`. Maker **Progress** dev-env ribbon and **Launch scorecard** show `put_ui_flow_id`, `slice_e2e_passed`, and UI failure step/locator. Factory **T2b** cadence runs UI regression after PUT HTTP E2E. PR **unit** CI runs [`scripts/ci/run_framework_pack_ci_gate.py`](scripts/ci/run_framework_pack_ci_gate.py) (detect + all seven SPA PUT-preview packs + static HTML with keyboard/mouse fidelity; `playwright` dev dependency), [`scripts/ci/run_bootstrap_ci_gate.py`](scripts/ci/run_bootstrap_ci_gate.py) (wheel build + isolated install smoke including fresh-venv subprocess), and [`scripts/ci/run_publish_bootstrap_ci_gate.py`](scripts/ci/run_publish_bootstrap_ci_gate.py) (publish workflow parity); weekly **fullstack-weekly-soak** via [`scripts/ops/run_fullstack_weekly_soak.py`](scripts/ops/run_fullstack_weekly_soak.py). Env: `NIMBUSWARE_LAUNCH_TEST_ENABLED`, `NIMBUSWARE_LAUNCH_TEST_WRITER_MODEL` (with `NIMBUSWARE_USE_LLM=1`).
- **Human-fidelity + diagnose/learn** — lang/heading/perf-budget smoke, optional axe-core rule packs (`NIMBUSWARE_AXE_ENABLED=1`), keyboard-nav, and optional negative-login paths (`human_fidelity.py`); `dev_env.human_fidelity` stage on [`micro_slice_fullstack`](configs/workflows/micro_slice_fullstack.yaml) (frozen as `dev_env_effective.human_fidelity_enabled` on `run.created`); failure learnings under `docs/learnings/` with stack fingerprint (`diagnose_learn.py`); learnings API + Maker panel; ADR [011](docs/adr/011-human-fidelity-e2e.md), [012](docs/adr/012-diagnose-fix-learn.md).
- **Operator interjection + autopilot (trust slider)** — Next/Last interjection queue; message prefixes `[patch]`, `[steer]`, `[skip]`, `[build]`; **trust/autopilot slider** 0–10 (`autopilot_profiles.py`, [`configs/autopilot/presets.yaml`](configs/autopilot/presets.yaml)); work-type defaults on `run.created` (patch → **8 Nimble**, factory → **10 Continuous improve**); saved profiles + Settings default for Chat; ribbons on Progress and Chat; slice-cycle wiring in `slice_cycle_integration.py`; ADR [013](docs/adr/013-operator-interjection.md)–[015](docs/adr/015-custom-autopilot-profiles.md), [020](docs/adr/020-unified-chat-work-type-routing.md)
- **Patch lane** (`workflow_profile=patch`, [`configs/workflows/patch.yaml`](configs/workflows/patch.yaml); stack variants [`patch_go.yaml`](configs/workflows/patch_go.yaml), [`patch_jvm.yaml`](configs/workflows/patch_jvm.yaml)) — hotfix profile: one bounded slice, minimal stage graph (plan → implement → verify → test → gate, no E2E), targeted test from `patch_context`; headless CI entry: [docs/deploy/headless-patch-ci.md](docs/deploy/headless-patch-ci.md); escalation to micro-slice offered in chat when patch gate fails; frozen `work_type` / `work_type_source` on `run.created` metadata.
- **Code intelligence + continuous improvement** — in-repo code graph, orphan/similarity/cohesion indexes (`code_graph.py`, `orphan_index.py`, `similarity_index.py`, `cohesion_graph.py`); `inventory_health_score` in `repo_inventory.py` weights improvement-council votes (`improvement_council.py`, including `research_transplant` from stitch catalog or pattern index — skips when no source); `simplification.rubric` on SIMPLIFY track and `refactor.loc_accord` on refactor passes; allowlisted graph tools (`repo_graph_tools.py`, incl. `list_module_deps` in repo explore); repo explorer and variant arena with crossover merge + slice-cycle promotion (`repo_explorer.py`, `variant_arena.py`); heuristic resolution council with loc accord, optional soak metrics, and remediable gate blocks (`resolution_council.py` — see [ADR 019](docs/adr/019-debate-first-resolution.md)); council ribbons in theater summarize gate outcomes; ADR [016](docs/adr/016-repo-exploration-variants.md)–[019](docs/adr/019-debate-first-resolution.md).
...
Configs live under [`configs/`](configs/) (workflows, personas, roles, `model-routing.yaml` including `ollama_user_policy`, bundles, `critic_packs/` including industry packs `fintech-api` and `healthcare-api` with `review_rules`, `skills/`). Critic packs expose `GET /v1/config/critic-packs/{id}/workflows` for workflow blast-radius preview. With Postgres, operator edits persist to `nimbusware_config_document` and materialize at API startup (optional git export via `nimbusware-config`). Bundle catalog authority is YAML under the repo root unless `NIMBUSWARE_DATABASE_URL` is set, in which case `policy/bundle-catalog` in Postgres is authoritative (`GET /v1/bundles/catalog/source`). Bootstrap wheel publish: [`.github/workflows/publish_bootstrap.yml`](.github/workflows/publish_bootstrap.yml) (`workflow_dispatch`; set `publish_pypi=true` only when `PYPI_API_TOKEN` is configured in repo secrets). Consumer wheel: [`packages/nimbusware_bootstrap/README.md`](packages/nimbusware_bootstrap/README.md). Clean-VM plan: `python scripts/install_nimbusware.py --consumer-plan`.
...
|------|---------|---------|
| `NIMBUSWARE_SLICE_BUDGET_PRESET` | standard | Context caps: packet, repo map, symbol sketch, LLM history, handoff, memory excerpt |
...
| `NIMBUSWARE_THEATER_LLM_SUMMARY` | off | Append optional theater summary when `1` or run metadata `theater.llm_summary` (LLM when `NIMBUSWARE_USE_LLM=1`, else rules digest) |
| `NIMBUSWARE_THEATER_LLM_MODEL` | llama3.2 | Ollama model for theater summary when LLM summary is enabled |
...
- Script: [`scripts/benchmarks/swe_bench_harness.py`](scripts/benchmarks/swe_bench_harness.py)
...
**One-command path** (clone + non-interactive, no Postgres):
...
Smoke check (no GUI): `python run.py --smoke` or `python scripts/ops/e2e_smoke.py`. Use `--admin --smoke` for the admin web entry.
...
- Congruent thread: classifier, run status, steering, and live **run theater** with `actor_display`, severity, and evidence toggles in collapsible **run cards**; session sidebar lists `GET /v1/chat/sessions` per project; trust/interjection ribbons when `?run_id=` is set; fork from any user turn with optional replay alignment on switch-mode; indented branch tree from graph API ([ADR 021](docs/adr/021-conversation-dag-branching.md))
- **Theater modes:** follow-live (cap 96 lines) or digest (12 lines); full archive + `GET /v1/runs/{id}/theater/export`
- First-visit **autonomy ladder** hint (patch → micro-slice → factory); ties to live trust slider when a run is active
- Settings → **Chat** (resume session) and **Trust / Autopilot** (default saved profile for Chat start)
...
- **Run theater** group chat on Progress tab and inline in Chat run cards (`GET /v1/runs/{id}/theater`, SSE `/theater/stream` with `actor_display`, `headline` / `body_md`, markdown export `/theater/export`); shared renderer in `theater-renderer.js`
...
- **Chat workspace** — session sidebar, run cards with trust level, branch tree, follow-live theater, operator ribbons (interjection + trust slider)
- **Trust defaults** — patch runs start at Nimble (8); factory at Continuous improve (10); Settings saves default profile for Chat
- **Gate-fail recover** — findings workspace with evidence + failed gate steps on Progress; Review approval cards link to recover on Progress
- **Plan steer** — pending backlog slices offer steer-to-Chat interjection draft
...
- **Findings** — blocking findings panel on Progress (HIGH/BLOCKER severity with repro steps and evidence)
...
- **Compliance bundle** — per-run `GET /v1/runs/{id}/audit-export` (gzip: events, policy snapshot, theater transcript); download from Review git panel or Admin run detail
...
| **Memory influence** | `GET /runs/{id}/memory-influence` | User |
...
Scripts: [`scripts/faiss/build_bundle_faiss_index.py`](scripts/faiss/build_bundle_faiss_index.py), [`scripts/faiss/build_memory_faiss_index.py`](scripts/faiss/build_memory_faiss_index.py), [`scripts/ops/run_dispatch_worker.py`](scripts/ops/run_dispatch_worker.py), [`scripts/ops/prune_scraper_artifacts.py`](scripts/ops/prune_scraper_artifacts.py), [`scripts/ops/e2e_smoke.py`](scripts/ops/e2e_smoke.py).

Runbooks: [`scripts/runbooks/run_dispatch_fleet_runbook.md`](scripts/runbooks/run_dispatch_fleet_runbook.md), [`scripts/runbooks/fleet_ollama_sli_runbook.md`](scripts/runbooks/fleet_ollama_sli_runbook.md).
...
Layered operator testing lives under `tests/e2e/` (**50** journey tests with `-m e2e_journey`; one slow stack soak is opt-in). Playwright checks visible Build/Review/Progress/Settings controls via route activation (`tests/e2e/web/maker_route_helper.ts`); **55** tests in **41** spec files cover apply-slice, session hub, launch scorecard (per-dimension rows), Settings launch check + **hybrid routing presets**, Admin operator chat classifier cards, Admin hardware catalog strip, Admin fleet hardware table + rescan, Admin critic packs config tab, context artifact save/insert, integrator stitch ribbon, **operator ribbons** (dev-env, interjection, autopilot, variant arena fitness), **memory influence table**, **council ribbon gaps**, Admin launch scorecard + factory evidence, scoped compaction toolbar, multi-slice campaign progress, mobile theater parity, home intents + factory hero demos, Progress gate-fail action cards, Chat branch active-leaf switch, Chat route-backed theater SSE (`event: theater` + `headline`/`body_md`), Chat autopilot ladder hint, and full campaign+scorecard replay. Launch-test journeys cover unknown SPA synthesis, static HTML packs, all nine framework detect paths, and write-replan with keyboard plus optional mouse fidelity (`NIMBUSWARE_MOUSE_FIDELITY=1`). Postgres e2e stack journeys verify chat session persistence, analytics across API restarts, and live theater SSE. Launch-eval parity rows map to specs via [`tests/web/parity_launch_wiring.yaml`](tests/web/parity_launch_wiring.yaml); Chat parity rows via [`tests/web/parity_chat_wiring.yaml`](tests/web/parity_chat_wiring.yaml) with an **≥80%** wired-row gate; Progress parity via [`tests/web/parity_progress_wiring.yaml`](tests/web/parity_progress_wiring.yaml); Admin parity via [`tests/web/parity_admin_wiring.yaml`](tests/web/parity_admin_wiring.yaml); IDE MCP parity via [`tests/web/parity_ide_wiring.yaml`](tests/web/parity_ide_wiring.yaml); congruent-thread journeys via [`tests/web/chat_journey_scenarios.yaml`](tests/web/chat_journey_scenarios.yaml). Go/JVM patch gate journeys run in [`.github/workflows/patch_toolchain.yml`](.github/workflows/patch_toolchain.yml). Stack subprocess tests set `NIMBUSWARE_EMBED_DISPATCH_WORKER=1` so the API process drains the memory queue; long-run stack soak: [`scripts/runbooks/e2e_stack_soak_runbook.md`](scripts/runbooks/e2e_stack_soak_runbook.md) (weekly **stack-soak** job in [`.github/workflows/slow_tests.yml`](.github/workflows/slow_tests.yml)). Redis fleet dispatch soak: [`scripts/runbooks/e2e_redis_fleet_soak_runbook.md`](scripts/runbooks/e2e_redis_fleet_soak_runbook.md) (weekly **redis-fleet-soak** with dual Redis brokers via `NIMBUSWARE_REDIS_FLEET_URLS`). Persistent dev-env journey soak: [`scripts/ops/run_dev_env_weekly_soak.py`](scripts/ops/run_dev_env_weekly_soak.py) (weekly **dev-env-weekly-soak**). Integration tests with live Redis use `tests/integration/test_redis_dispatch_worker_stack.py` (`-m integration`). The default unit CI job runs the `slice.e2e` apply journey when `NIMBUSWARE_SLICE_E2E_COMMAND` is set. PR **e2e** job retries flaky journeys once (`--reruns 1`); weekly [`.github/workflows/e2e_flake_monitor.yml`](.github/workflows/e2e_flake_monitor.yml) re-runs the same Postgres e2e suite and opens an issue on failure.
...
poetry run python scripts/benchmarks/launch_eval.py path/to/workspace --json --llm   # opt-in Ollama findings
poetry run python scripts/benchmarks/launch_eval.py --matrix   # score all catalog default_workspaces
poetry run python scripts/benchmarks/launch_eval.py --run-id <uuid> --json   # Postgres attach + attach_context
poetry run python scripts/benchmarks/launch_eval.py --run-id <uuid> --run-events events.json --json
```
...
./scripts/ci/ci_check.sh    # Linux/macOS (--skip-web to skip vitest/Playwright)
...
Optional hooks: `pip install pre-commit && pre-commit install` (ruff, format, compileall, mypy via `scripts/ci/mypy_ci_targets.py` tranches B–E).

Integration tests need `NIMBUSWARE_DATABASE_URL` (`@pytest.mark.integration`). Gates script: `scripts/ci/run_integration_like_ci.ps1` / `.sh`.
...
| `NIMBUSWARE_SLICE_BUDGET_PRESET` | user | Micro-slice context caps: `tiny`, `standard`, or `careful` (replaces per-key `*_MAX_CHARS`) |
...
Full catalog: `poetry run python scripts/ci/audit_operator_env.py` (237 keys).
