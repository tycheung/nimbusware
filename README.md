...
- **Human-fidelity + diagnose/learn** — negative-path and keyboard-nav smoke (`human_fidelity.py`); failure learnings under `docs/learnings/` with stack fingerprint (`diagnose_learn.py`); ADR [011](docs/adr/011-human-fidelity-e2e.md), [012](docs/adr/012-diagnose-fix-learn.md).
- **Operator interjection + autopilot** — Next/Last interjection queue (`interjection_queue.py`, `GET/POST /v1/runs/{id}/interjection-queue`); autopilot slider presets 0–10 and custom checkpoints (`autopilot_profiles.py`, [`configs/autopilot/presets.yaml`](configs/autopilot/presets.yaml), `GET /v1/autopilot/presets/{level}`); ADR [013](docs/adr/013-operator-interjection.md)–[015](docs/adr/015-custom-autopilot-profiles.md).
- **Code intelligence + continuous improvement** — in-repo code graph, orphan/similarity/cohesion indexes (`code_graph.py`, `orphan_index.py`, `similarity_index.py`, `cohesion_graph.py`); repo explorer and variant arena (`repo_explorer.py`, `variant_arena.py`); improvement council (`improvement_council.py`, `repo_inventory.py`); debate-first resolution council before remediable gate blocks (`resolution_council.py`); ADR [016](docs/adr/016-repo-exploration-variants.md)–[019](docs/adr/019-debate-first-resolution.md).
