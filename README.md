...
**Enterprise** — self-hosted control plane for governed agentic development: audit every run, fleet compliance dashboard (gate pass rates, slice histogram), tenant audit policy with **legal-hold toggle** (blocks event-store purge), fleet governance summary in Maker Home (allowed deploy targets, mandatory discovery fields, deploy approval chain, compliance summary widget), deploy credential scope audit (`.nimbusware/platform/deploy_audit.jsonl` + Review timeline API), steer autonomy, standardize quality without a SaaS black box.
