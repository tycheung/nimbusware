...
Default CI runs **~3355** unit tests under the default pytest marker (plus **106** Playwright tests including product-polish smoke); full collection is **~4256** tests. CI enforces prune-comments, explainer-export lint, workflow-explainer init sync, **workflow YAML line budget** (optional profiles should use `extends: default` — feature toggles and `nimbusware_production` were collapsed in Jul 2026), **standards stream-guardrails** (architecture + complexity via `packages/standards`), archetype-fit, gate-comprehension, and collab-LLM audit gates (`scripts/ci/run_*_ci_gate.py`; LOC budget ceiling **102,713** non-blank Python lines in `packages/` per `scripts/ci/loc_baseline.json`). Optional fast gate bundle: `python scripts/ci/fast_gates.py` (architecture + complexity streams). Context efficiency (progressive compaction, cache-aware prompts, index-first memory) is documented in [ARCHITECTURE.md](ARCHITECTURE.md#context-efficiency-jul-2026). Standards bundles and facades: [ADR 029](docs/adr/029-standards-ci-streams.md), [ADR 030](docs/adr/030-standards-bundles-mart.md).
