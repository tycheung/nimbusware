...
Default CI runs **~3700** unit tests under the default pytest marker (plus **106** Playwright tests including product-polish smoke); full collection is **~4600** tests. CI enforces prune-comments, explainer-export lint, workflow-explainer init sync, **workflow YAML line budget** (optional profiles should use `extends:` with fragments under `configs/workflows/fragments/`), **composite contract test size** (all `test_*composite_contract*.py` files ≤200 lines via matrix modules under `tests/unit/composite_contracts/` — allowlist empty Jul 2026), **parallel standards streams** (eight `streams` matrix jobs + aggregate — the `test` stream is omitted because the `unit` job is the single pytest run; local `ci_check` sets `NIMBUSWARE_CI_STREAMS_SKIP_TEST=1` for the same reason), archetype-fit, gate-comprehension, and collab-LLM audit gates (`scripts/ci/run_*_ci_gate.py`; LOC budget ceiling **102,713** non-blank Python lines in `packages/` per `scripts/ci/loc_baseline.json`). Optional fast gate bundle: `python scripts/ci/fast_gates.py` (architecture + complexity streams). Maker Progress includes a **Standards** ribbon (facade, bundles, run-now); Admin includes a **Standards mart** browser. Context efficiency (progressive compaction, cache-aware prompts, index-first memory) is documented in [ARCHITECTURE.md](ARCHITECTURE.md#context-efficiency-jul-2026). Standards bundles and facades: [ADR 029](docs/adr/029-standards-ci-streams.md), [ADR 030](docs/adr/030-standards-bundles-mart.md). Language- and rigor-aware bundle defaults: `configs/standards/preset_defaults.yaml`.

**Workflow profiles:** prefer thin overlays in `configs/workflows/` with `extends: default` (or another base). Full profiles belong on the allowlist in `scripts/ci/workflow_yaml_allowlist.txt`. Model routing canonical source is `configs/model-routing.yaml`.
