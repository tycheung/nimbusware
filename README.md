...
Default CI runs **~3355** unit tests under the default pytest marker (plus **106** Playwright tests including product-polish smoke); full collection is **~4198** tests. CI enforces prune-comments, explainer-export lint, workflow-explainer init sync, archetype-fit, gate-comprehension, and collab-LLM audit gates (`scripts/ci/run_*_ci_gate.py`; LOC budget ceiling **103,672** non-blank Python lines in `packages/` per `scripts/ci/loc_baseline.json`). Admin OpenAPI TypeScript types are generated at build/CI (`packages/nimbusware_admin_ui`: `npm run codegen:openapi`), not committed.
