...
Default CI runs **~3490** unit tests under the default pytest marker (plus **106** Playwright tests including product-polish smoke); full collection is **~4260** tests. CI enforces prune-comments, explainer-export lint, workflow-explainer init sync, **workflow YAML line budget** (optional profiles should use `extends:` with fragments under `configs/workflows/fragments/`), **composite contract test size** (all `test_*composite_contract*.py` files ≤200 lines via matrix modules under `tests/unit/composite_contracts/` — allowlist empty Jul 2026), **parallel standards streams** (nine `streams` matrix jobs + aggregate; local `ci_check` runs `run_all_streams.py --profile nimbusware-monorepo` with `NIMBUSWARE_CI_STREAMS_SKIP_TEST=1` so pytest is not run twice), archetype-fit, gate-comprehension, and collab-LLM audit gates (`scripts/ci/run_*_ci_gate.py`; LOC budget ceiling **102,713** non-blank Python lines in `packages/` per `scripts/ci/loc_baseline.json`). Optional fast gate bundle: `python scripts/ci/fast_gates.py` (architecture + complexity streams). Maker Progress includes a **Standards** ribbon (facade, bundles, run-now); Admin includes a **Standards mart** browser. Context efficiency (progressive compaction, cache-aware prompts, index-first memory) is documented in [ARCHITECTURE.md](ARCHITECTURE.md#context-efficiency-jul-2026). Standards bundles and facades: [ADR 029](docs/adr/029-standards-ci-streams.md), [ADR 030](docs/adr/030-standards-bundles-mart.md). Language- and rigor-aware bundle defaults: `configs/standards/preset_defaults.yaml`.
