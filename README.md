...
Default CI runs **~3355** unit tests under the default pytest marker (plus **106** Playwright tests including product-polish smoke); full collection is **~4258** tests. CI enforces prune-comments, explainer-export lint, workflow-explainer init sync, **workflow YAML line budget** (optional profiles should use `extends: default` — nine feature toggles were collapsed in Jul 2026), archetype-fit, gate-comprehension, and collab-LLM audit gates (`scripts/ci/run_*_ci_gate.py`; LOC budget ceiling **102,847** non-blank Python lines in `packages/` per `scripts/ci/loc_baseline.json`). CI prints a JSON LOC summary via `scripts/ci/run_loc_report.py`. Admin OpenAPI TypeScript types are generated at build/CI (`packages/nimbusware_admin_ui`: `npm run codegen:openapi`), not committed.
