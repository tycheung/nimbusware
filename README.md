...
Environment toggles use the **`NIMBUSWARE_*`** prefix (~207 catalog keys across install/system/user/run scopes). Read via [`packages/nimbusware_env/env_flags.py`](packages/nimbusware_env/env_flags.py) and [`docs/operator-settings.md`](docs/operator-settings.md) — not raw `os.environ` in feature code. See [`.env.example`](.env.example).
