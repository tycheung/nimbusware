...
Default CI runs **~3280** unit tests under the default pytest marker (plus **106** Playwright tests including optional product-polish smoke); full collection is **~4000** tests. CI enforces prune-comments, explainer-export lint, workflow-explainer init sync, archetype-fit, gate-comprehension, and collab-LLM audit gates (`scripts/ci/run_*_ci_gate.py`; LOC budget **103,662** non-blank Python lines in `packages/` per `scripts/ci/loc_baseline.json`, ceiling **103,672**).
