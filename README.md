...
- **Factory scaffold (v2 preview)** — PUT preview runtime (`put_runtime.py`: stack detection, local preview subprocess), factory tiers T0–T3 (`factory_completion.py`, [`configs/factory/factory_tier_policy.yaml`](configs/factory/factory_tier_policy.yaml)), interaction surface map (`interaction_surface_map.py`), and PUT E2E flow runner (`put_e2e_runner.py`, [`configs/factory/flows/`](configs/factory/flows/)); golden replay via [`tests/fixtures/factory/golden_factory_replay.json`](tests/fixtures/factory/golden_factory_replay.json) (`contacts_api` flow).
