...
**CI parity:** `ci_check.*` runs ruff check + **blocking** format, openapi TS gate (full schema when Node present), publish VS Code gate, mypy (targets above), bandit (`pyproject.toml`), pip-audit, framework-pack gate (keyboard/mouse fidelity), package coverage floors, pytest @ 75% (**~4198** collected / **~3355+** in the default unit job, ≥75% line coverage; packages LOC baseline **103,672** max via `scripts/ci/loc_baseline.json` — current **~102,824**; module size guards cover orchestrator, API, memory, projections (≤450 lines) and console (≤400); see `tests/README.md`). Workflow block parsers for micro-slice, theater, dev-env, fast-slice, escalation, and integration-adapter-writer live in `workflow_blocks_simple.py` (thin `workflow_*.py` re-export shims removed Jul 2026). `load_create_run_workflow_blocks` uses `workflow_registry.parse_workflow_block`; campaign requirement helpers live in `runs/create.py`; chat scope discovery routes mount from `chat_scope_discover.py`; deploy policy helpers colocate in `platform_deploy_mutations.py`; seven workflow explainers use YAML metric specs under `configs/explainers/`.
