...
**CI layout (PR):** `.github/workflows/ci.yml` runs **stream-guardrails** (architecture + complexity streams via `run_all_streams.py --profile nimbusware-core`), then **unit** (ruff, `audit_operator_env`, openapi TS gate, publish VS Code gate, mypy, bandit, pip-audit, pytest @ 75%, framework-pack + SLO gates), **web** (full OpenAPI schema regen, vitest maker + admin, Playwright `tests/e2e/web`), **integration**, and **e2e** jobs. Local parity: `scripts/ci/ci_check.ps1` / `ci_check.sh` (unit + optional web when Node is present).
...
| Provider cache plumbing | `cache_blocks` → resolver → HTTP | `orchestrator/routing/resolver.py`, `llm/prompt_cache.py` |
| Persisted token samples | `context.budget.sampled` events (30s rate limit) | `orchestrator/llm/budget_sample_emit.py` |
| Incremental Progress SSE | Tail fetch + `progress_delta` events | `store.list_run_events_since`, `api/routes/runs/stream.py` |
| Memory index skip-rebuild | Unchanged source event fingerprint | `memory/index/fingerprint.py`, `memory/index/indexer.py` |
| Campaign artifact bundle | Implement-safe export (no chat transcript) | `GET /v1/runs/{id}/campaign-artifact-bundle` |

Cache tiers: `agent_core/prompt_tiers.py` (`assemble_prompt_with_cache_metadata`); provider headers: `orchestrator/llm/prompt_cache.py` (multi-block Anthropic T0/T1). Memory index-first injection: `memory/index/index_table.py` + `NIMBUSWARE_MEMORY_INDEX_FIRST`; rebuild skips when `source_events_fingerprint` matches manifest. Read modes: `outline`, `digest`, `full` for Python, TypeScript/JavaScript, and Go via `agent_core/read_outline.py`. Maker Progress context chip shows persisted `token_samples` and offload savings (`render-chips.js`). Maker Progress SSE uses tail event fetch and client-side delta merge (`progress.js`).
