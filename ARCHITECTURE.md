...
**CI parity:** `ci_check.*` runs ruff check + **blocking** format, mypy (targets above), bandit (`pyproject.toml`), pip-audit, package coverage floors, pytest @ 75% (~2,803 tests / 3,663 collected, 82% line coverage Jun 2026). CI mypy targets pass with **0** errors. Shared helpers: `agent_core.mapping` (`mapping_or_empty`, `load_error_text`), `gate_fail_for_stage` / `ollama_runtime_from_host` in `_pipeline/_helpers.py`, `micro_slice_run_context` for run-created metadata, table-driven universal-critique env resolution, `_context_max_chars` in `env_flags.py`. Maturity backlog: local `plan_gap.md` (gitignored).
