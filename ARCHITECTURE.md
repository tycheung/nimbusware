...
| `nimbusware_console` | Admin display helpers + enterprise fleet formatters; workflow explainers use `explainer_core/schema_metrics`, `explainer_core/env_captions`, `explainer_core/env_summaries`, and `workflow_exports`; BFF via `admin_ui_bff.py` |
...
**CI parity:** `ci_check.*` runs ruff check + **blocking** format, openapi TS gate (full schema when Node present), publish VS Code gate, mypy (targets above), bandit (`pyproject.toml`), pip-audit, framework-pack gate (keyboard/mouse fidelity), package coverage floors, pytest @ 75% (~3028 unit tests in the default job / 3873 collected, ≥75% line coverage; packages LOC baseline **94,789** via `scripts/ci/loc_baseline.json`; see `tests/README.md`).
