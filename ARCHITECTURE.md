...
Cache tiers: `agent_core/prompt_tiers.py` (`assemble_prompt_with_cache_metadata`); provider headers: `orchestrator/llm/prompt_cache.py` (multi-block Anthropic T0/T1). Memory index-first injection: `memory/index/index_table.py` + `NIMBUSWARE_MEMORY_INDEX_FIRST`. Read modes: `outline`, `digest`, `full` via `agent_core/read_outline.py`. Maker Progress SSE uses tail event fetch and client-side delta merge (`maker_web/static/js/tabs/progress.js`). Stage plugin contract: `orchestrator/_pipeline/stage_dispatch.py` (`PIPELINE_STAGES`).
