...
**CI parity:** `ci_check.*` runs ruff check + **blocking** format, mypy (targets above), bandit (`pyproject.toml`), pip-audit, package coverage floors, pytest @ 75%. Mypy debt is tracked in local `plan_gap.md` (**S9**); Jun 2026 burn-down reduced CI errors from ~362 → ~135 (OpenAPI `__all__`, dev-env payloads, no-any-return/union-attr clusters).
