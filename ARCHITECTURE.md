...
**CI layout (PR):** `.github/workflows/ci.yml` runs **standards streams** (nine parallel jobs + aggregate), then **unit** (ruff, prune/trim comment gates, mypy, bandit, pip-audit, pytest @ 75%, product SLO gates), **web** (vitest maker + admin, Playwright), **integration**, and **e2e**. Local parity: `run_all_streams.py --profile nimbusware-monorepo` plus `scripts/ci/ci_check.ps1` / `ci_check.sh`.
