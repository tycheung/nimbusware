...
**Release v1 operator surfaces:** Maker Home intents + factory catalog demos; Chat autonomy ladder hint + escalation; PR-on-complete and factory evidence scorecard (JSON + HTML + zip) on Review; Progress `gate_summary` + `role_cost_summary` + learnings ribbon + factory tier promotion; hybrid routing presets; external CI on integrator / `slice.gate` / `factory.gate`; headless patch CI; `patch_go` / `patch_jvm` + launch-eval catalog entries; regulated buyer bundle in `docs/enterprise-buyer.md`; interjection latency SLO on Admin timeline; Enterprise fleet autopilot tenant policies (`configs/enterprise/fleet_autopilot_policies.yaml`); external chat webhook steering (`[patch]`/`[steer]`/`/status`). Ledger: local `plan_gap.md` (gitignored).
