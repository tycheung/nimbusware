...
**Release v1 operator surfaces:** Maker Home intents + factory catalog demos; Chat autonomy ladder hint + escalation; PR-on-complete and factory evidence scorecard on Review; Progress `gate_summary` + `role_cost_summary` + learnings ribbon on block; hybrid routing presets; external CI on integrator / `slice.gate` / `factory.gate`; headless patch CI; `patch_go` / `patch_jvm` + launch-eval catalog entries; regulated buyer bundle in `docs/enterprise-buyer.md`. Ledger: local `plan_gap.md` (gitignored).
