...
**CI parity:** `ci_check.*` runs ruff check + **blocking** format, mypy (targets above), bandit (`pyproject.toml`), pip-audit, package coverage floors, pytest @ 75%. CI mypy targets pass with **0** errors. Shared helpers: `agent_core.mapping` (`mapping_or_empty`, `load_error_text`), `gate_fail_for_stage` / `ollama_runtime_from_host` in `_pipeline/_helpers.py`, table-driven universal-critique env resolution, `_context_max_chars` in `env_flags.py`, `_uc_payload` in UC workflow explainers. Maturity backlog: local `plan_gap.md` (audit **#10** — **E1**–**E8**; **E4**–**E3** done in P1–P3).
