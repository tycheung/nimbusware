...
**CI parity:** `ci_check.*` runs ruff check + **blocking** format, openapi TS gate (full schema when Node present), publish VS Code gate, mypy (targets above), bandit (`pyproject.toml`), pip-audit, framework-pack gate (keyboard/mouse fidelity), package coverage floors, pytest @ 75% (~3201 unit tests in the default job / ~4000 collected, ≥75% line coverage; packages LOC baseline **100,764** via `scripts/ci/loc_baseline.json`; module size guards cover orchestrator, API, memory, projections (≤450 lines) and console (≤400); see `tests/README.md`).
