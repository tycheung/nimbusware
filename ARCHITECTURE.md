...
`hermes_orchestrator._pipeline.*` mixins stay `ignore_errors = true` except the strict islands above (`protocol_hosts.py` documents `CritiqueGateHost`, `MicroSliceHost`, `LifecycleVerifyHost`, `WritersHost`). API lifespan and the run worker share `hermes_orchestrator.runtime_bootstrap.build_runtime_orchestrator`.
