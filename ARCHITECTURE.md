...
**CI parity:** `ci_check.*` runs ruff check + **blocking** format, openapi TS gate (full schema when Node present), publish VS Code gate, mypy (targets above), bandit (`pyproject.toml`), pip-audit, framework-pack gate (keyboard/mouse fidelity), package coverage floors, pytest @ 75% (**~4197** collected / **~3279+** in the default unit job, ≥75% line coverage; packages LOC baseline **103,672** max via `scripts/ci/loc_baseline.json` — current **~102,822**; module size guards cover orchestrator, API, memory, projections (≤450 lines) and console (≤400); see `tests/README.md`). Workflow block parsers for micro-slice, theater, dev-env, fast-slice, escalation, and integration-adapter-writer live in `workflow_blocks_simple.py` (thin `workflow_*.py` re-export shims removed Jul 2026). Chat scope discovery routes are mounted from `chat_session.py`; seven workflow explainers use YAML metric specs under `configs/explainers/`.
