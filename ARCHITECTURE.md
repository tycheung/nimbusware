...
**CI parity:** `ci_check.*` runs ruff check + **blocking** format, mypy (targets above), bandit (`pyproject.toml`), pip-audit, package coverage floors, pytest @ 75%. CI mypy targets pass with **0** errors. Shared helpers: `agent_core.mapping` (`mapping_or_empty`, `load_error_text`), table-driven universal-critique env resolution, `_context_max_chars` in `env_flags.py`, `_uc_payload` in UC workflow explainers. Env consolidation epics: local `plan_gap.md` (**S3**–**S9**).
